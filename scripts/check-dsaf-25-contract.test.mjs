...
function fixtureRepo() {
  ...
  writeFileSync(join(root, "docs/03-criteria-part-a.md"), `| # | Criterion | Tag | 0 | 3 | 5 |\n|---|---|---|---|---|---|\n${tableRows(partA)}\n`);
  writeFileSync(join(root, "docs/04-criteria-part-b.md"), `| # | Criterion | Tag | 0 | 3 | 5 |\n|---|---|---|---|---|---|\n${tableRows(partB)}\n`);
  writeFileSync(join(root, "docs/dsaf-25.md"), `---\ndsaf_125_version: "test"\n---\nIt is not DSAF Lite.\ndsaf_25_score%\nfull DSAF Criteria\nPublication cap\n| # | Source | Category | Criterion | Tag |\n|---:|---|---|---|---|\n${coreRows(rows)}\n`);
  writeFileSync(join(root, "docs/dsaf-25-card.md"), `DSAF-25 Core is the shareable entry point\n${rows.map((row, index) => `${index + 1}. ${row.id} ${row.name}`).join("\n")}\nUse the full DSAF Criteria for signed audits.\n`);
  writeFileSync(join(root, "assets/dsaf-25-card.svg"), `<svg width="210mm" height="297mm" viewBox="0 0 210 297" role="img"><title>DSAF-25</title><desc>${rows.map((row) => `${row.id} ${row.name}`).join("; ")}</desc>${rows.map((row) => `<text>${row.id}</text>`).join("")}</svg>`);
  writeFileSync(join(root, "assets/dsaf-25-card-print.pdf"), "x".repeat(1200));
  writeFileSync(join(root, "landing/card/index.html"), `<svg aria-labelledby="t d"><title id="t">DSAF-25 Core</title><desc id="d">DSAF-25 Core</desc>${rows.map((row) => `<text>${row.id}</text>`).join("")}</svg>`);
  writeFileSync(join(root, "templates/audit-report-template.md"), "dsaf_25_score: 0\n");
...
test("evaluateDsaf25Contract catches verbatim criterion drift", () => {
  const root = fixtureRepo();
  try {
    const text = readFileSync(join(root, "docs/dsaf-25.md"), "utf8").replace("Canonical A1.1", "Reworded A1.1");
    writeFileSync(join(root, "docs/dsaf-25.md"), text);
...
test("evaluateDsaf25Contract catches missing category coverage", () => {
  const root = fixtureRepo();
  try {
    const text = readFileSync(join(root, "docs/dsaf-25.md"), "utf8").replace("| 25 | B10.1 | Category B10.1 | Canonical B10.1 | FIXED |", "| 25 | B9.1 | Category B9.1 | Canonical B9.1 | FIXED |");
    writeFileSync(join(root, "docs/dsaf-25.md"), text);
    const failures = evaluateDsaf25Contract(payload, root).results.filter((item) => item.status === "fail");
