...
export const PAYLOAD_PATH = resolve(ROOT, "docs/branding/FR-BRAND-001-domain-contract.json");
export const AUDIT_OUTPUT = resolve(ROOT, "docs/_audit/domain-contract.json");
...
export function evaluateLiveSnapshot(snapshot, payload) {
  ...
  results.push(check(snapshot.landing.body.includes(`<link rel="canonical" href="${canonical.url}">`), "landing canonical link", "present", `<link rel=\"canonical\" href=\"${canonical.url}\">`));
  results.push(check(snapshot.landing.body.includes(`<meta property="og:url" content="${canonical.url}">`), "landing og:url", "present", `<meta property=\"og:url\" content=\"${canonical.url}\">`));
