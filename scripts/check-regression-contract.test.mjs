...
function fixtureRepo() {
  ...
  writeFileSync(join(root, "docs/regression-policy.md"), "Regressions are allowed only when they are visible, attributed, and signed.\n`rubric-tightened` `fix-side-effect` `external-dependency-change` `deliberate-policy-tradeoff` `D-RT` `OVRD-FSE` `OVRD-EDC` `OVRD-DPT` `UNRESOLVED`\nOlder audits with `no_downgrade: true` remain valid\n");
  writeFileSync(join(root, "docs/02-framework.md"), `## §4 No-silent-regression rule\n${common}\nAlways honour the no-silent-regression rule\n`);
  writeFileSync(join(root, "docs/06-fix-cycle.md"), common);
  writeFileSync(join(root, "docs/07-maturity-tiers.md"), "DSAF Level transitions and the no-silent-regression rule\nevery regression in a signed audit is surfaced with a cause, tag, and approval path\nD-RT\n");
  writeFileSync(join(root, "templates/audit-report-template.md"), common);
  writeFileSync(join(root, "prompts/fix-mode.md"), "No-silent-regression gate override_log Do not auto-revert. Rollback is a human decision. RE_AUDIT (awaiting override)\n");
  ...
  writeFileSync(join(root, "docs/feature-requests/old.md"), "no-downgrade rule is allowed here by exclusion");
  return root;
}
...
test("walkPolicyFiles honors exclusions", () => {
  ...
  try {
    ...
    assert.ok(!files.includes("docs/feature-requests/old.md"));
  }
  finally {
...
test("evaluateRegressionContract catches legacy term drift", () => {
  const root = fixtureRepo();
  try {
    writeFileSync(join(root, "docs/02-framework.md"), "no-downgrade rule\n");
...
test("evaluateRegressionContract catches missing package scripts", () => {
  const root = fixtureRepo();
  try {
