...
function fixtureRepo() {
  ...
  mkdirSync(join(root, "examples/cyberskill-design-system"), { recursive: true });
  ...
  writeFileSync(join(root, "docs/03-criteria-part-a.md"), table(partAIds));
  writeFileSync(join(root, "docs/04-criteria-part-b.md"), table(partBIds));
  writeFileSync(join(root, "docs/criteria-aliases.md"), `# DSAF criterion aliases\nMerged-away ID\nPrimary ID\nAlias IDs are never reused.\nA8 Accessibility vs B5 Accessibility & Inclusive\nA1 Tokens vs A8 accessibility tokens\n| Merged-away ID | Primary ID | Merged date | Rationale |\n|---|---|---:|---|\n${payload.expected_aliases.map((row) => `| ${row.merged_away_id} | ${row.primary_id} | 2026-05-18 | rationale |`).join("\n")}\n`);
  writeFileSync(join(root, "docs/criteria-dedup-methodology.md"), "Dedup passes consolidate true duplicates without renumbering surviving IDs.\nCompare pairs within the same Part only.\nDo not merge FIXED with DYNAMIC.\nDo not merge across Part A and Part B.\nDo not leave any category empty.\nThis is the stable DSAF-125 baseline.\n");
  writeFileSync(join(root, "docs/dsaf-25.md"), `| # | Source | Category | Criterion | Tag |\n|---:|---|---|---|---|\n${[...partAIds, ...partBIds].slice(0, 25).map((id, index) => `| ${index + 1} | ${id} | Cat | Name ${id} | FIXED |`).join("\n")}\n`);
  writeFileSync(join(root, "examples/cyberskill-design-system/improvement-plan.md"), "No alias references here.\n");
  writeFileSync(join(root, "examples/cyberskill-design-system/_history.md"), "P0 FR-CORE-003 Criterion rows reduced from 138 to exactly 125 13 merged-away IDs preserved\n");
...
test("evaluateCriteriaDedupContract catches reused alias IDs", () => {
  const root = fixtureRepo();
  try {
    writeFileSync(join(root, "docs/03-criteria-part-a.md"), readFileSync(join(root, "docs/03-criteria-part-a.md"), "utf8") + criterionLine("A10.4") + "\n");
...
test("evaluateCriteriaDedupContract catches broken primaries and chains", () => {
  const root = fixtureRepo();
  try {
    const aliases = readFileSync(join(root, "docs/criteria-aliases.md"), "utf8")
      .replace("| A10.4 | A10.3 |", "| A10.4 | A10.6 |");
    writeFileSync(join(root, "docs/criteria-aliases.md"), aliases);
    const failures = evaluateCriteriaDedupContract(payload, root).results.filter((item) => item.status === "fail");
...
test("evaluateCriteriaDedupContract catches missing package scripts", () => {
  const root = fixtureRepo();
  try {
...
test("evaluateCriteriaDedupContract reports missing artefacts", () => {
  const root = fixtureRepo();
  try {
    rmSync(join(root, "docs/criteria-dedup-methodology.md"), { force: true });
    const failures = evaluateCriteriaDedupContract(payload, root).results.filter((item) => item.status === "fail");
