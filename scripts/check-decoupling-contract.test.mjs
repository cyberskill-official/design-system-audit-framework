...
function fixtureRepo() {
  ...
  mkdirSync(join(root, "docs/_audit"), { recursive: true });
  mkdirSync(join(root, "landing/card"), { recursive: true });
  mkdirSync(join(root, "landing/benchmark"), { recursive: true });
  mkdirSync(join(root, "landing/benchmark/privacy"), { recursive: true });
  mkdirSync(join(root, ".github"), { recursive: true });

  writeFileSync(join(root, "docs/branding/decoupling-decision.md"), [
  ...
  writeFileSync(join(root, "docs/branding/domain-decision.md"), [
  ...
  writeFileSync(join(root, "docs/branding/url-redirect-map.md"), [
  ...
  writeFileSync(join(root, "docs/ADR-FR-BRAND-004.md"), "FR-BRAND-004\n\n**Status:** accepted\n\n```mermaid\nflowchart TD\nA-->B\n```\n");
  writeFileSync(join(root, ".github/CODEOWNERS"), payload.required_codeowners.join("\n"));
  writeFileSync(join(root, "landing/vercel.json"), JSON.stringify({ headers: [{ headers: [{ key: "Strict-Transport-Security" }, { key: "Content-Security-Policy" }] }] }));
  writeFileSync(join(root, "README.md"), "https://audit.cyberskill.world\nSERVICES.md\nmethodology surface stays neutral\n");
  writeFileSync(join(root, "landing/index.html"), "<link rel=\"canonical\" href=\"https://audit.cyberskill.world/\"><meta property=\"og:url\" content=\"https://audit.cyberskill.world/\">DSAF — Design System Audit Framework");
  writeFileSync(join(root, "landing/card/index.html"), "<link rel=\"canonical\" href=\"https://audit.cyberskill.world/card\">DSAF-25 Core card");
  writeFileSync(join(root, "landing/benchmark/index.html"), "<link rel=\"canonical\" href=\"https://audit.cyberskill.world/benchmark/\">Benchmark");
  writeFileSync(join(root, "landing/benchmark/results.html"), "<link rel=\"canonical\" href=\"https://audit.cyberskill.world/benchmark/results.html\">Results");
  writeFileSync(join(root, "landing/benchmark/privacy/index.html"), "<link rel=\"canonical\" href=\"https://audit.cyberskill.world/benchmark/privacy/\">info@cyberskill.world");
  writeFileSync(join(root, "landing/README.md"), "`https://audit.cyberskill.world`\nNo paid-service CTA, pricing, or sales form lives here.\nNo redirect rules ship from here\n");
...
test("walkActiveSurfaces includes active landing files", () => {
  const root = fixtureRepo();
  try {
    mkdirSync(join(root, "landing", "dist"), { recursive: true });
    mkdirSync(join(root, "landing", ".cache"), { recursive: true });
    writeFileSync(join(root, "landing", "dist", "ignored.html"), "https://dsaf.dev");
    writeFileSync(join(root, "landing", ".cache", "ignored.html"), "https://dsaf.dev");
    writeFileSync(join(root, "landing", "ignored.json"), "{\"url\":\"https://dsaf.dev\"}");
    const files = walkActiveSurfaces(root, payload).map((file) => file.replace(root + "/", ""));
    assert.ok(files.includes("README.md"));
    assert.ok(files.includes("landing/index.html"));
    assert.ok(!files.includes("landing/dist/ignored.html"));
    assert.ok(!files.includes("landing/.cache/ignored.html"));
    assert.ok(!files.includes("landing/ignored.json"));
  }
  finally {
...
test("evaluateDecouplingContract catches stale neutral-domain links and sales copy", () => {
  const root = fixtureRepo();
  try {
    writeFileSync(join(root, "landing/benchmark/results.html"), "See https://dsaf.dev and book a paid audit.");
...
test("evaluateDecouplingContract catches redirect config and malformed inventory", () => {
  const root = fixtureRepo();
  try {
    writeFileSync(join(root, "landing/vercel.json"), JSON.stringify({ redirects: [{ source: "/x", destination: "/y" }], headers: [] }));
    writeFileSync(join(root, "docs/branding/url-redirect-map.md"), "no rows");
    const failures = evaluateDecouplingContract(payload, root).results.filter((item) => item.status === "fail");
...
test("evaluateDecouplingContract catches missing governance and package scripts", () => {
  const root = fixtureRepo();
  try {
