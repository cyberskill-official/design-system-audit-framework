...
function fixtureRepo() {
  ...
  mkdirSync(join(root, "docs/framework/examples/cyberskill-design-system"), { recursive: true });
  const files = [
  ...
  writeFileSync(join(root, "docs/internal/SERVICES.md"), "CyberSkill cites the worked example at L3.\n");
  writeFileSync(join(root, "docs/internal/branding/self-audit-policy.md"), "No third-party verification -> L3. Combined percentages may appear inside audit data tables.\n");
  writeFileSync(join(root, "docs/guidelines/01-introduction.md"), "## The self-audit publication cap\nThis caps at L3.\n");
  writeFileSync(join(root, "docs/framework/07-maturity-tiers.md"), "## Self-audit cap rule\nL3 (Managed) maximum without third-party verification.\n");
  writeFileSync(join(root, "docs/framework/examples/cyberskill-design-system/README.md"), "Public cited level: L3 Managed.\n");
  writeFileSync(join(root, "docs/framework/examples/cyberskill-design-system/improvement-plan.md"), "Worked Example Audit (L3 self-audit, uncertified)\nCited public Level\nL3 (Managed)\n");
  writeFileSync(join(root, "docs/framework/examples/cyberskill-design-system/_history.md"), "L3 capped\nself-audit cap\n");
  writeFileSync(join(root, "docs/framework/examples/cyberskill-design-system/audit-report-2026-04-27.md"), "combined: 84.6\ntier: L5 Optimised\n| Post-audit combined score | **84.6%** (L5 Optimised) |\n");
  return root;
}
...
test("walkPublicFiles excludes the preserved interior audit", () => {
  ...
  try {
    ...
    assert.ok(!files.includes("docs/framework/examples/cyberskill-design-system/audit-report-2026-04-27.md"));
  }
  finally {
