...
function fixtureRepo() {
  ...
  mkdirSync(join(root, "docs/framework/assets/og"), { recursive: true });
  mkdirSync(join(root, "apps/landing/assets/og"), { recursive: true });
  mkdirSync(join(root, "apps/landing/blog/launch-2026"), { recursive: true });
...
test("evaluateLaunchBlogContract catches missing files and package docs/framework/scripts", () => {
...
test("writeLaunchBlogEvidence writes structured output and summarizes", () => {
  ...
  try {
    ...
    assert.equal(audit.task_id, "TASK-DOCS-003");
