...
test("evaluateCriteriaDedupContract catches reused alias IDs", () => {
  const root = fixtureRepo();
  try {
    writeFileSync(join(root, "docs/framework/03-criteria-part-a.md"), readFileSync(join(root, "docs/framework/03-criteria-part-a.md"), "utf8") + criterionLine("A10.4") + "\n");
...
test("evaluateCriteriaDedupContract catches broken primaries and chains", () => {
  const root = fixtureRepo();
  try {
    const aliases = readFileSync(join(root, "docs/framework/criteria-aliases.md"), "utf8")
      .replace("| A10.4 | A10.3 |", "| A10.4 | A10.6 |");
    writeFileSync(join(root, "docs/framework/criteria-aliases.md"), aliases);
    const failures = evaluateCriteriaDedupContract(payload, root).results.filter((item) => item.status === "fail");
...
test("evaluateCriteriaDedupContract catches missing package docs/framework/scripts", () => {
  const root = fixtureRepo();
  try {
...
test("evaluateCriteriaDedupContract reports missing artefacts", () => {
  const root = fixtureRepo();
  try {
    rmSync(join(root, "docs/framework/criteria-dedup-methodology.md"), { force: true });
    const failures = evaluateCriteriaDedupContract(payload, root).results.filter((item) => item.status === "fail");
