...
export const PAYLOAD_PATH = resolve(ROOT, "docs/branding/FR-BRAND-004-decoupling-contract.json");
export const AUDIT_OUTPUT = resolve(ROOT, "docs/_audit/decoupling-contract.json");
...
export function walkActiveSurfaces(root, payload) {
  ...
  function walk(abs) {
    ...
    if (rel.includes("node_modules") || rel.includes("docs/feature-requests/")) return;
    if (stat.isDirectory()) {
...
export function evaluateDecouplingContract(payload, root = ROOT) {
  ...
  if (adr !== null) {
    results.push(makeResult(adr.includes("FR-BRAND-004") && adr.includes("Status:** accepted") && adr.includes("```mermaid"), "adr-decision-shape", payload.files.adr_file, "accepted ADR with Mermaid", "present"));
  }
