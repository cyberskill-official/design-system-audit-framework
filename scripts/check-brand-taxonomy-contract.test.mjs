...
import {
  ...
  walkFiles,
  writeTaxonomyAudit,
} from "./brand-taxonomy-contract-lib.mjs";
...
function fixtureRepo() {
  ...
  writeFileSync(join(root, "CONTRIBUTING.md"), "Use DSAF Criteria and DSAF Levels.\n");
  writeFileSync(join(root, "SERVICES.md"), "Commercial work is separate from DSAF.\n");
  writeFileSync(join(root, "landing/index.html"), "<title>DSAF — Design System Audit Framework</title>\n");
  writeFileSync(join(root, "docs/branding/handle-taxonomy.md"), "DSAF Design System Audit Framework DSAF Criteria DSAF-25 Core DSAF Levels DSAF Modes SCAN mode FIX mode W mode DSAF DSAF DSAF DSAF\n");
  writeFileSync(join(root, "docs/branding/glossary.md"), "DSAF Design System Audit Framework DSAF Criteria DSAF-25 Core DSAF Levels DSAF Modes SCAN mode FIX mode W mode DSAF DSAF DSAF DSAF\n");
  return root;
}
...
test("walkFiles honors extensions and exclusions", () => {
  ...
  try {
    mkdirSync(join(root, "docs/feature-requests"), { recursive: true });
    writeFileSync(join(root, "docs/feature-requests/example.md"), "DSAF Framework example");
    const files = walkFiles(root, payload).map((file) => file.replace(root + "/", ""));
    assert.ok(files.includes("README.md"));
    assert.ok(!files.includes("docs/feature-requests/example.md"));
  }
  finally {
...
test("evaluateTaxonomy catches banned noun handles", () => {
  const root = fixtureRepo();
  try {
    writeFileSync(join(root, "landing/index.html"), "The DSAF Framework should fail.\n");
    const evaluation = evaluateTaxonomy(payload, root);
    const failures = evaluation.results.filter((item) => item.status === "fail");
    assert.ok(failures.some((item) => item.file === "landing/index.html"));
  }
  finally {
    rmSync(root, { recursive: true, force: true });
  }
