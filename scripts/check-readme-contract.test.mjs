...
<picture>
  ...
  <img src="./assets/dsaf-l0-l5-ladder.svg" alt="DSAF Levels ladder" width="100%">
</picture>

<picture>
  <source srcset="./assets/dsaf-radar-dark.svg" media="(prefers-color-scheme: dark)">
  <img src="./assets/dsaf-radar.svg" alt="DSAF radar chart" width="100%">
</picture>

**Read DSAF-25 Core first.** If you only have 5 minutes, [docs/dsaf-25.md](./docs/dsaf-25.md) is the 25-criterion subset that fits on one page. The full DSAF Criteria are in docs.
...
npm run verify
node scripts/audit-init.mjs /tmp/system
\`\`\`

Then open [prompts/scan-mode.md](./prompts/scan-mode.md).

## Reading Order

| # | File | Purpose |
|---|---|---|
| 1 | [docs/01-introduction.md](./docs/01-introduction.md) | Extended introduction |
| 2 | [docs/02-framework.md](./docs/02-framework.md) | Modes |
| 3 | [docs/dsaf-25.md](./docs/dsaf-25.md) | DSAF-25 Core |
| 4 | [docs/05-running-an-audit.md](./docs/05-running-an-audit.md) | Workflow |
| 5 | [docs/07-maturity-tiers.md](./docs/07-maturity-tiers.md) | DSAF Levels |
| 6 | [prompts/scan-mode.md](./prompts/scan-mode.md) | SCAN |
| 7 | [docs/08-improvement-plan.md](./docs/08-improvement-plan.md) | Plan |

## External Review Status

Named outside-reviewer quotes are not published until explicit written consent is logged. See docs/branding/reviewer-consent-log.md.
...
DSAF Modes, Complete L3 self-audit example, self-audit publication policy, SERVICES.md, methodology surface stays neutral.
`;
}
...
function fixtureRepo() {
  ...
  writeFileSync(join(root, "docs/01-introduction.md"), payload.reading_order.intro_required.join("\n"));
...
test("evaluateReadmeContract catches reading order and endorsement regressions", () => {
  const root = fixtureRepo();
  try {
    const broken = fixtureReadme().replace("| 7 | [docs/08-improvement-plan.md](./docs/08-improvement-plan.md) | Plan |\n", "").replace(/> "<endorsement quote[^\n]+\n/g, "");
    writeFileSync(join(root, "README.md"), broken);
    writeFileSync(join(root, "docs/01-introduction.md"), "missing intro rows");
    const failures = evaluateReadmeContract(payload, root).results.filter((item) => item.status === "fail");
