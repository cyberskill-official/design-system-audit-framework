...
export function evaluateLiveSnapshot(snapshot, payload) {
  ...
  results.push(check(snapshot.landing.body.includes(`<link rel="canonical" href="${canonical.url}">`), "apps/landing canonical link", "present", `<link rel=\"canonical\" href=\"${canonical.url}\">`));
  results.push(check(snapshot.landing.body.includes(`<meta property="og:url" content="${canonical.url}">`), "apps/landing og:url", "present", `<meta property=\"og:url\" content=\"${canonical.url}\">`));
