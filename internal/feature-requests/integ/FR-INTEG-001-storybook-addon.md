...
export interface CriterionScore {
  /** Criterion ID per framework/03-criteria-part-a.md or framework/04-criteria-part-b.md (e.g., "A.1.1") */
  id: string;
 ...
 * Per FR-CORE-001 §1 verbatim-quote rule: criterion IDs + names match framework/03-criteria-part-a.md and framework/04-criteria-part-b.md verbatim.
 */

import type { CriterionScore, RunnerResult, PanelData } from './types.js';

/** Known criterion metadata — kept in sync with framework/03-criteria-part-a.md + framework/04-criteria-part-b.md.
...
- **About the criterion metadata:** the `CRITERION_METADATA` const in `scoring.ts` is the source of truth for criterion ID → name + tag + deep-dive URL mapping. It needs updating per FR-CORE-003 dedup (criterion IDs may change) + per FR-GOV-003 P6 RFCs (new criteria). Future automation: generate `CRITERION_METADATA` from `framework/03-criteria-part-a.md` + `framework/04-criteria-part-b.md` at addon build time.
