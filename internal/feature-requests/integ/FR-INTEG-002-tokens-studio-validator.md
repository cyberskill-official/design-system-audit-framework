...
**Why 9 validators specifically (§1 #3):** A.1 has 9 criteria (A.1.1-A.1.9) per `framework/03-criteria-part-a.md`. One validator per criterion = clear mapping; the validator's `CriterionScore[]` output aligns 1:1 with the rubric. Combining validators (e.g., "color + typography in one") would obscure the per-criterion mapping.
