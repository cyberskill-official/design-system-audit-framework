...
source_pages:
  ...
  - "framework/07-maturity-tiers.md (L0–L5 rubric — source of truth for the ladder)"
  - "framework/dsaf-25.md (DSAF-25 Core — source of category dimensions for the radar)"
source_decisions:
...
new_files:
  ...
  - internal/branding/FR-BRAND-003-visual-assets-contract.json
...
sub_tasks:
  ...
  - "6. (30m) Patch guidelines/01-introduction.md to reference the visuals at the relevant moment in the prose"
  - "7. (30m) Patch framework/07-maturity-tiers.md to embed the L0-L5 ladder near §1 'The six tiers' table"
  - "8. (30m) Patch framework/dsaf-25.md to embed the radar template near the 'How to use' section"
...
14. **MUST** ensure the L0–L5 ladder visually communicates the *transition gates* (what it takes to climb from L1 to L2, L4 to L5, etc.) — not just the tier names. The ladder MAY use connecting arrows, gate labels, or per-tier "you need:" callouts to convey the climb path documented in `framework/07-maturity-tiers.md` §3.
15. **MUST** make the radar visually communicate the *enterprise-grade thresholds* — the floors documented in `framework/07-maturity-tiers.md` §2 (`A.8` ≥ 75%, `B.5` ≥ 75%, `A.1` ≥ 70%, `A.4` ≥ 60%, `A.3` ≥ 65%, any single category ≥ 40%). The radar SHOULD overlay a "minimum enterprise" boundary as a dashed line so a viewer sees at a glance whether a given audit clears all floors.
...
- One-line meaning extracted from `framework/07-maturity-tiers.md` §1
...
### `guidelines/01-introduction.md` — patch (visual reference at "What you'll produce" section)
...
### `framework/07-maturity-tiers.md` — patch (embed ladder at §1 "The six tiers")
...
### `framework/dsaf-25.md` — patch (embed radar template near "How to use")
...
11. **`framework/07-maturity-tiers.md` embeds ladder** — `grep -q 'dsaf-l0-l5-ladder' framework/07-maturity-tiers.md`.
12. **`framework/dsaf-25.md` references radar template** — `grep -q 'dsaf-radar' framework/dsaf-25.md && grep -q 'dsaf-radar-template.json' framework/dsaf-25.md`.
13. **`guidelines/01-introduction.md` references both visuals** — `grep -q 'dsaf-l0-l5-ladder' guidelines/01-introduction.md && grep -q 'dsaf-radar' guidelines/01-introduction.md`.
...
grep -q 'dsaf-l0-l5-ladder' framework/07-maturity-tiers.md

# AC12 — dsaf-25 references radar template
grep -q 'dsaf-radar' framework/dsaf-25.md && grep -q 'dsaf-radar-template.json' framework/dsaf-25.md

# AC13 — intro references both
grep -q 'dsaf-l0-l5-ladder' guidelines/01-introduction.md && grep -q 'dsaf-radar' guidelines/01-introduction.md
...
7. **(30m) Patch `guidelines/01-introduction.md`** to reference both visuals at the "What you'll produce" section.
8. **(30m) Patch `framework/07-maturity-tiers.md`** to embed the ladder at §1.
9. **(30m) Patch `framework/dsaf-25.md`** to reference the radar template at "How to use."
...
- **Why the design spec lives in `assets/`, not `internal/branding/`:** the spec is a designer's reference, not a general-doctrine surface. Placing it alongside the SVGs keeps the implementation surface co-located. A future co-maintainer or commissioned illustrator gets one folder to read.
