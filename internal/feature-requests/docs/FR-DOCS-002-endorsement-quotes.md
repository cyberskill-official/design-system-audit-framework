...
effort_hours: 3
sub_tasks:
  - "1. (15m) Confirm at least 2 entries in internal/branding/reviewer-quotes-pending.md have matching consent-log entries from FR-GOV-001"
  ...
  - "5. (15m) Update internal/branding/reviewer-shortlist.md: status moves from 'quote-approved' to 'quote-published' for each landed quote"
...
1. **MUST** replace the README's endorsement placeholder slots with named quotes from `internal/branding/reviewer-quotes-pending.md`. Each placeholder `> "<endorsement quote, ≤ 280 chars>" — <Reviewer Name>, <Affiliation>` becomes `> "<actual approved quote>" — <Reviewer Name>, <Reviewer Affiliation>`. The replacement is mechanical; the content is from FR-GOV-001.
...
3. **MUST** verify byte-identical match between the quote text in `internal/branding/reviewer-quotes-pending.md` and the quote text published in README. No silent edits (typo fixes, punctuation tweaks, word substitutions). A reviewer who finds their published quote different from their consent-approved version is a relationship break. The verification is mechanical: `diff <(extract README quote) <(extract pending-quotes file quote)` returns empty.
...
9. **MUST** update `internal/branding/reviewer-shortlist.md` per landed quote: the row's status moves from `quote-approved` to `quote-published` with the publication date appended.
...
*Note: the actual quote text + attribution comes from `internal/branding/reviewer-quotes-pending.md` per FR-GOV-001 consent letters — the operator at PR land time replaces the illustrative quotes above with the real ones. The illustrative quotes are NOT to be published as-is; they're authoring scaffolding.*

### `internal/branding/reviewer-shortlist.md` — status transitions
...
5. **Byte-identical match to consent-log** — for each published quote, `internal/branding/reviewer-quotes-pending.md` (or `internal/branding/reviewer-consent-log.md`) contains a record with the same byte-identical quote text. `diff <(extract README quotes) <(extract consent-log quotes)` returns empty for matched pairs.
...
8. **Shortlist status updated** — for each landed quote, `internal/branding/reviewer-shortlist.md` shows the row's status as `quote-published (<YYYY-MM-DD>)` per §3.
9. **No third quote without consent** — number of quotes in README equals the number of `quote-approved` or `quote-published` entries in `internal/branding/reviewer-shortlist.md`.
...
quote_published_rows=$(grep -c 'quote-published' internal/branding/reviewer-shortlist.md)
...
quote_approved_or_published=$(grep -cE 'quote-(approved|published)' internal/branding/reviewer-shortlist.md)
...
1. **(15m) Verify consent.** Open `internal/branding/reviewer-quotes-pending.md` (or wherever FR-GOV-001 stored approved quotes). Confirm at least 2 entries have matching consent-log entries (per FR-GOV-001 §3 consent letter signed-and-archived discipline).
...
5. **(15m) Update `internal/branding/reviewer-shortlist.md`** — for each landed quote, change row status from `quote-approved` to `quote-published (<YYYY-MM-DD>)`.
...
1. Nathan Curtis — consent-log entry: internal/branding/reviewer-consent-log.md row 2026-06-04 — approved as written.
2. Sil Bormüller — consent-log entry: internal/branding/reviewer-consent-log.md row 2026-06-05 — approved with affiliation correction ("Founder, Into Design Systems" — added 'Founder' per her request).
