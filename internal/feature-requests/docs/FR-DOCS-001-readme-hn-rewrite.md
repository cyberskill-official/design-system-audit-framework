...
modified_files:
  - guidelines/01-introduction.md  # cross-link adjustments after README becomes the canonical entry point
  - package.json
  - scripts/dsaf-verify.mjs
allowed_tools:
  - "file_read/write README.md, guidelines/01-introduction.md"
...
sub_tasks:
  ...
  - "5. (45m) Cross-link patches — guidelines/01-introduction.md Reading Order updated; ensure README references all canonical surfaces (dsaf-25.md, regression-policy.md, self-audit-policy.md, decoupling-decision.md)"
...
2026-05-18 strict execution note: stale status reset; `npm run contract:readme` verifies the README launch-copy contract: first-200-word what/why-now/comparison pitch, above-the-fold ladder/radar embeds, DSAF-25 cross-link, Quick Start, Reading Order, endorsement placeholder consent guard, handle taxonomy, banned funnel-copy patterns, and the mocked colleague-skim request/response shape. It writes `outputs/_audit/readme-contract.json`. The original `dsaf.dev` canonical requirement is superseded by the FR-BRAND-004 canonical-host override.
...
4. **MUST** include the "Read DSAF-25 Core first" cross-link block per FR-CORE-001 §3 — the one that says "If you only have 5 minutes, [`framework/dsaf-25.md`](./framework/dsaf-25.md) is the 25-criterion subset that fits on one page." This block MUST appear within the first 600 words of README body copy.
...
**Why README-as-landing-page (§1 #1, #14):** the plan §"What drives GitHub stars" item 1 is explicit — "A README that reads like a finished product, not documentation — repo IS the landing page for HN." Show HN traffic arrives at the README; they don't click through. A README that opens with "Welcome to the project. To get started, see guidelines/01-introduction.md..." loses the HN reader in 5 seconds. A README that opens with the pitch + visuals + Quick Start in the first 600 words converts.
...
Full audit walkthrough: [`guidelines/05-running-an-audit.md`](./guidelines/05-running-an-audit.md).
...
| # | File | Purpose |
|---|---|---|
| 1 | [`guidelines/01-introduction.md`](./guidelines/01-introduction.md) | What DSAF is, who it's for, what you'll produce |
| 2 | [`framework/02-framework.md`](./framework/02-framework.md) | Modes, actors, scoring, no-silent-regression rule |
| 3 | [`framework/dsaf-25.md`](./framework/dsaf-25.md) | **DSAF-25 Core — the 25-criterion subset on one page (recommended entry point)** |
| 4 | [`guidelines/05-running-an-audit.md`](./guidelines/05-running-an-audit.md) | Step-by-step playbook |
| 5 | [`framework/07-maturity-tiers.md`](./framework/07-maturity-tiers.md) | What each DSAF Level means |
| 6 | [`prompts/scan-mode.md`](./prompts/scan-mode.md) | Paste this into your LLM and run your first SCAN |
| 7 | [`guidelines/08-improvement-plan.md`](./guidelines/08-improvement-plan.md) | Turn an audit into a phased improvement plan |

For the criteria themselves: [`framework/03-criteria-part-a.md`](./framework/03-criteria-part-a.md) (Part A — System) and [`framework/04-criteria-part-b.md`](./framework/04-criteria-part-b.md) (Part B — UX).

For tailoring DSAF to your industry / brand: [`guidelines/09-customising.md`](./guidelines/09-customising.md).

For the LLM prompt pack: [`guidelines/10-prompt-pack.md`](./guidelines/10-prompt-pack.md).
...
Branding: [`internal/branding/`](./docs/branding) — handle taxonomy, decoupling decision, self-audit publication policy.
...
### `guidelines/01-introduction.md` — Reading Order patch (post-README rewrite)
...
5. **(45m) Cross-link patches.** Apply the `guidelines/01-introduction.md` Reading Order patch from §3. Verify that all README cross-links resolve (relative paths to existing files).
