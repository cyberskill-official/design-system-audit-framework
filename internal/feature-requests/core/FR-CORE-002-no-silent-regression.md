...
source_pages:
  ...
  - "framework/02-framework.md §4 (current no-downgrade rule)"
  - "framework/07-maturity-tiers.md §5 (tier-transitions interaction)"
source_decisions:
...
sub_tasks:
  ...
  - "3. (45m) patch framework/02-framework.md §4 (rule rename + new mechanic body)"
  - "4. (30m) patch framework/07-maturity-tiers.md §5 (tier-transitions section coordinates with FR-BRAND-002's text-only patches)"
  - "5. (30m) patch guidelines/06-fix-cycle.md (the FIX cycle's verification step now surfaces + requires override, not rollback)"
...
1. **MUST** rename the rule from `no-downgrade rule` to `no-silent-regression rule` across every doctrine surface: `framework/02-framework.md` §4, `guidelines/06-fix-cycle.md`, `framework/07-maturity-tiers.md` §5, `internal/branding/glossary.md` (per FR-BRAND-002), `templates/audit-report-template.md` frontmatter, `prompts/fix-mode.md`. The rename is global; no surface MAY keep "no-downgrade" as the primary handle. Older audits' frontmatter that cite `no_downgrade: true` MUST remain readable (backward-compatible reading); new audits emit `no_silent_regression: true` instead.
...
9. **MUST NOT** remove the rollback *option*. A team MAY explicitly choose to roll back an offending fix (the `@Human[rollback]` action tag in `framework/02-framework.md` §2 still applies). The rollback becomes a *human decision*, not an *automatic mechanic*. The §7 Verification block records which option the team chose.
...
12. **MUST** add a sentence to `framework/02-framework.md` §7 (Hard rules) clarifying that "always honour the no-silent-regression rule" replaces the prior "always honour the no-downgrade rule." Rule #8 in the existing list is the locus of the rename; surrounding rules don't shift.
...
**Why preserve the rollback *option* (§1 #9):** sometimes the right answer IS to revert the offending fix. The framework should NOT preclude that — it should just stop *automating* it. By making rollback a `@Human[rollback]` action (already in `framework/02-framework.md` §2's action-tag table), we preserve the discipline of "regressions cost the team something" without enforcing the discipline mechanically.
...
Pseudo-code for the agent's `VERIFYING` step in `guidelines/06-fix-cycle.md`:
...
### `framework/02-framework.md` §4 — patch (before / after)
...
### `framework/02-framework.md` §7 — patch (Hard rules item 8)
...
### `guidelines/06-fix-cycle.md` — patch (VERIFYING step)

The exact patch depends on the current text of `guidelines/06-fix-cycle.md` (operator MUST `Read` at land time per the FR-CORE-004 discipline). The change pattern:
...
### `framework/07-maturity-tiers.md` §5 — patch (coordinates with FR-BRAND-002)

FR-BRAND-002 already patches `framework/07-maturity-tiers.md` §5 heading text as part of the handle taxonomy. This FR's patch is body-content only — the rule rename inside §5:
...
**Companion to:** `framework/02-framework.md` §4 (rule statement) and `guidelines/06-fix-cycle.md` (FIX cycle mechanic).
...
3. **`framework/02-framework.md` §4 rewritten** — section title is `## §4 No-silent-regression rule`; body matches §3 "After" shape; four cause categories enumerated.
4. **`framework/02-framework.md` §7 rule 8 updated** — item 8 reads "Always honour the no-silent-regression rule" (not "no-downgrade").
5. **`guidelines/06-fix-cycle.md` patched** — VERIFYING step describes "draft override_log entry + pause at RE_AUDIT (awaiting override)"; no "automatic rollback" language remains.
6. **`framework/07-maturity-tiers.md` §5 patched** — body cites the no-silent-regression rule; the `[D-RT]` tag is mentioned at least once in §5.
...
9. **Four cause categories enumerated everywhere** — `grep -c 'rubric-tightened\|fix-side-effect\|external-dependency-change\|deliberate-policy-tradeoff'` returns ≥ 4 in each of `framework/02-framework.md`, `guidelines/06-fix-cycle.md`, `docs/regression-policy.md`, `templates/audit-report-template.md`, `prompts/fix-mode.md`.
10. **Six allowed tag values enumerated** — `framework/02-framework.md` §4 + `docs/regression-policy.md` + `templates/audit-report-template.md` each enumerate `D-RT`, `OVRD-RT`, `OVRD-FSE`, `OVRD-EDC`, `OVRD-DPT`, `UNRESOLVED`.
...
# AC3 — §4 rewritten
grep -q '^## §4 No-silent-regression rule' framework/02-framework.md
grep -q 'rubric-tightened\|fix-side-effect\|external-dependency-change\|deliberate-policy-tradeoff' framework/02-framework.md

# AC4 — §7 rule 8 updated
grep -q 'no-silent-regression rule' framework/02-framework.md

# AC5 — fix-cycle patched
grep -q 'override_log\|awaiting override' guidelines/06-fix-cycle.md
rg -ti md 'automatic rollback' guidelines/06-fix-cycle.md  # MUST be empty

# AC6 — maturity-tiers §5 patched
grep -q 'no-silent-regression' framework/07-maturity-tiers.md
grep -q 'D-RT' framework/07-maturity-tiers.md
...
for file in framework/02-framework.md guidelines/06-fix-cycle.md docs/regression-policy.md \
...
2. **(45m) Author `docs/regression-policy.md`** per §3.
3. **(45m) Patch `framework/02-framework.md` §4** (full body rewrite per §3) and §7 rule 8 (single-line rename).
4. **(30m) Patch `guidelines/06-fix-cycle.md`** VERIFYING step. `Read` the file at land time; apply the patch pattern from §3.
5. **(30m) Patch `framework/07-maturity-tiers.md` §5.** This patch coordinates with FR-BRAND-002's text-only patches to §5 (FR-BRAND-002 §3 table row "Tier transitions" handles the heading + adjacent prose; this FR handles the rule-body content).
...
- **Downstream blocks:** FR-DOCS-001 (README rewrite includes the new rule's tagline), FR-BRAND-002 (BRAND-002's §3 patch table to `framework/07-maturity-tiers.md` already coordinates the rename in headline text — this FR provides the body change).
...
- **External:** none. The framework's `@Human[approve]` and `@Human[rollback]` action tags from `framework/02-framework.md` §2 are pre-existing; this FR repurposes them, doesn't introduce them.
