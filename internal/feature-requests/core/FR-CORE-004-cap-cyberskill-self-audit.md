...
service: doctrine
new_files:
  - internal/branding/self-audit-policy.md
  - internal/branding/FR-CORE-004-self-audit-contract.json
...
sub_tasks:
  ...
  - "2. (30m) author internal/branding/self-audit-policy.md per §3"
  - "3. (45m) apply find/replace patches per §3 to README.md, guidelines/01-introduction.md, framework/07-maturity-tiers.md, examples/cyberskill-design-system/improvement-plan.md, examples/cyberskill-design-system/_history.md"
...
6. **MUST** add a section to `framework/07-maturity-tiers.md` titled "Self-audit cap rule" that codifies the cap-at-L3-without-verification rule. The section MUST cite `internal/branding/self-audit-policy.md` as the source of truth.
...
11. **MUST** add a one-paragraph rationale to the public README (or to `guidelines/01-introduction.md`) explaining the cap rule so external readers see it before they see CyberSkill's self-audit. The rationale exists to pre-empt the "auditor self-grades at L5" critique.
...
### `internal/branding/self-audit-policy.md` (NEW)
...
| `industry-leading` (in CyberSkill self-description) | `worked example at L3 (Managed); see [self-audit publication policy](./internal/branding/self-audit-policy.md)` |
...
### `guidelines/01-introduction.md` — additive section
...
### `framework/07-maturity-tiers.md` — additive section (between existing §2 and §3)
...
| YAML frontmatter `cited_tier: L5` or similar (if present) | `cited_tier: L3` (and add a sibling field `cited_tier_cap_rationale: "FR-CORE-004 + internal/branding/self-audit-policy.md"`) |
...
```markdown
| 2026-05-17 | FIX | DSAF v0 + FR-CORE-004 | Claude | Stephen Cheng | Stephen Cheng | <interior%> | **L3 (capped)** | Re-framed as worked example; interior scores intact; cap rationale: internal/branding/self-audit-policy.md |
```
...
1. **Self-audit policy committed** — `internal/branding/self-audit-policy.md` exists with the content shape in §3 (cap table, why-the-cap rationale, combined-percentage ban, dual-bookkeeping ban, cap-lift trigger, worked-example pointer, amendment clause).
...
3. **Introduction has cap section** — `guidelines/01-introduction.md` contains a section titled exactly `## The self-audit publication cap` with the body in §3.
4. **Maturity-tiers doc has cap section** — `framework/07-maturity-tiers.md` contains a section titled `## §2.5 Self-audit cap rule` (or equivalent numbering) with the body in §3.
...
10. **Cap-lift trigger documented** — `internal/branding/self-audit-policy.md` "Cap-lift trigger" section names the verifier-signature requirement and the cap-lift evidence file path (`_audit/verifications/`).
11. **README pre-emptive rationale present** — README first 200 words (after FR-DOCS-001 lands) OR `guidelines/01-introduction.md` first section contains the one-paragraph cap rationale pointing readers at the policy file before they encounter the worked example.
...
test -f internal/branding/self-audit-policy.md && grep -q '^## The cap rule' internal/branding/self-audit-policy.md
echo $?  # 0
...
grep -q '^## The self-audit publication cap' guidelines/01-introduction.md

# AC4 — maturity-tiers doc has cap section
grep -qE '^## §2\.5 Self-audit cap rule' framework/07-maturity-tiers.md
...
grep -q '^## The cap-lift trigger' internal/branding/self-audit-policy.md

# AC11 — pre-emptive rationale (depends on whether FR-DOCS-001 has shipped)
# If FR-DOCS-001 NOT yet shipped: check guidelines/01-introduction.md
grep -q 'cap rule\|capped at L3\|self-audit publication policy' guidelines/01-introduction.md
...
2. **(30 min) Author `internal/branding/self-audit-policy.md`.** Copy the body from §3 verbatim; commit.
3. **(30 min) Append cap section to `guidelines/01-introduction.md`** per §3.
4. **(15 min) Append cap section to `framework/07-maturity-tiers.md`** as §2.5 per §3.
...
**Cap rationale:** [self-audit publication policy](../../../framework/core/branding/self-audit-policy.md).
...
|---|---|---|---|---|---|---|---|
| 2026-05-17 | FIX | DSAF v0 + FR-CORE-004 | Claude | Stephen Cheng | Stephen Cheng | **L3 (Managed) per cap rule** | Re-framed as worked example; interior scores intact; cap rationale: internal/branding/self-audit-policy.md |
```
...
- **Q3: Where does the cap rule live — README, framework spec, separate policy file?** Resolved → separate policy file (`internal/branding/self-audit-policy.md`). The README references it; the framework spec (`framework/02-framework.md`) and maturity-tiers doc (`framework/07-maturity-tiers.md`) point at it; the policy file is the source of truth.
...
| Cap rule misread as "DSAF can't certify L5 ever" | reviewer feedback | Framework reads as defeatist | Rationale paragraph in `guidelines/01-introduction.md` "self-audit publication cap" section makes the cap-lift trigger explicit. The framework CAN certify L5; the path is FR-CERT-001 + L5-entry-gate stack |
...
| Cap policy file gets edited to remove the L3 rule by an unauthorised actor | git review at PR | Policy weakened silently | The policy file is normative (it says so in its header); changes require FR-GOV-003 (P6) RFC OR explicit operator approval recorded in `MEMORY.md`. CODEOWNERS for `internal/branding/` set to founder + future co-maintainer (post-FR-GOV-002) |
...
- **Read-before-patch discipline.** Operators landing this FR's patches MUST `Read` each touched file (README.md, guidelines/01-introduction.md, framework/07-maturity-tiers.md, examples/cyberskill-design-system/improvement-plan.md, examples/cyberskill-design-system/_history.md) at the time of patch authorship to enumerate the actual existing phrasings. The "Before" columns in §3 tables are illustrative — they describe the *kinds* of phrasings to match, not necessarily the literal strings present in the current files. Read-before-patch prevents the find/replace from being applied against guessed text.
- **Service surface extension.** The frontmatter `service: doctrine` is shorthand for "this FR ships markdown changes, not code." The actual files touched span `docs/`, `README.md`, `examples/cyberskill-design-system/`, and (via §1 #14 forward-binding) any future SCAN/FIX runs that re-audit the worked example. CODEOWNERS for `internal/branding/` and `examples/cyberskill-design-system/` should both be set to the founder + future co-maintainer (post-FR-GOV-002) to prevent silent relaxation.
...
- **Why CODEOWNERS for `internal/branding/`:** the cap policy file is the kind of thing that gets quietly relaxed under sales pressure. Naming the founder (and future co-maintainer per FR-GOV-002) as the CODEOWNER for `internal/branding/` makes "lift the cap" a deliberate two-person decision rather than a single-author oversight.
