...
modified_files:
  - framework/03-criteria-part-a.md
  - framework/04-criteria-part-b.md
  - framework/dsaf-25.md      # may need ID updates if Part B renumbers (per FR-CORE-001 §3b caveat)
...
sub_tasks:
  ...
  - "5. (1h) Apply merges to framework/03-criteria-part-a.md and framework/04-criteria-part-b.md (verbatim before/after diffs in PR description)"
  - "6. (45m) Validate FR-CORE-001's DSAF-25 Core selection against the post-dedup Part B IDs; amend framework/dsaf-25.md if any Core row's Source ID changed"
...
**2026-05-18 strict execution note:** stale status was reset and FR-CORE-003 was re-processed with an executable criteria-dedup contract. `npm run contract:criteria-dedup` verifies that the live rubric stays at 125 rows, all 20 category prefixes remain populated, 13 merged-away IDs resolve to live primaries without alias chains or ID reuse, and DSAF-25/example surfaces do not cite aliases. It writes `outputs/_audit/criteria-dedup-contract.json`.

## §1 — Description (BCP-14 normative)

The 125-criterion rubric MUST undergo a dedup pass before P0 launch. The pass produces: (a) a list of merged criteria with primary surviving ID + merged-away alias ID; (b) an aliases file that maps every merged-away ID to its primary; (c) updates to `framework/03-criteria-part-a.md` and `framework/04-criteria-part-b.md` reflecting the merges; (d) a methodology file that makes future dedups deterministic.
...
9. **MUST** validate FR-CORE-001's DSAF-25 Core selection against the post-dedup Part A + Part B ID sets. Specifically: for every row in `framework/dsaf-25.md`, the `Source` column ID MUST resolve to a *primary* (surviving) criterion ID. If a DSAF-25 Source ID was merged away in this dedup pass, the DSAF-25 row's Source MUST be updated to the new primary ID in the same PR. The illustrative Part B IDs in FR-CORE-001 §3b (`B1.1`, `B2.1`, etc.) get stabilised here.
...
- Part A criteria: [`framework/03-criteria-part-a.md`](./03-criteria-part-a.md)
- Part B criteria: [`framework/04-criteria-part-b.md`](./04-criteria-part-b.md)
...
- Apply Step 4 (rubric folding) of FR-CORE-003 §3b to the surviving criterion's row in `framework/03-criteria-part-a.md` or `framework/04-criteria-part-b.md`.
...
- Re-run FR-CORE-001's DSAF-25 Core selection against the post-dedup ID set. Update `framework/dsaf-25.md` Source IDs if any Core row's Source was merged away.
...
### `framework/03-criteria-part-a.md` and `framework/04-criteria-part-b.md` — patches
...
### `framework/dsaf-25.md` — Source ID validation patch

For every row in `framework/dsaf-25.md`'s 25-row Core table, re-check the `Source` column ID against the post-dedup primary ID set:
...
5. **Every category retained ≥ 1 criterion** — `grep -c '^| A1\.' framework/03-criteria-part-a.md` ≥ 1 for every A.1 … A.10; same for B.1 … B.10.
...
7. **No FIXED-DYNAMIC merges** — for every alias entry, the surviving criterion's tag matches the merged-away criterion's tag (verify via cross-reference to `framework/03-criteria-part-a.md` / `framework/04-criteria-part-b.md`).
...
9. **DSAF-25 Source IDs validated** — every row in `framework/dsaf-25.md`'s Core table has a Source ID that resolves to a *primary* in the post-dedup rubric (not an alias). If any Source was merged away, the row's Source is updated to the new primary.
...
part_a_count=$(grep -cE '^\| A[0-9]+\.[0-9]+' framework/03-criteria-part-a.md)
part_b_count=$(grep -cE '^\| B[0-9]+\.[0-9]+' framework/04-criteria-part-b.md)
...
for cat in A1 A2 A3 A4 A5 A6 A7 A8 A9 A10 B1 B2 B3 B4 B5 B6 B7 B8 B9 B10; do
  if [ "${cat}" = "${cat#A}" ]; then file=framework/04-criteria-part-b.md; else file=framework/03-criteria-part-a.md; fi
...
sourced_ids=$(awk -F '|' '/^\| [0-9]+/ { print $3 }' framework/dsaf-25.md | tr -d ' ')
...
1. **(1h) Step 1 inventory.** `Read` `framework/03-criteria-part-a.md` and `framework/04-criteria-part-b.md` end-to-end. Tabulate every criterion in a scratch file with the 8 columns from §3a methodology Step 1.
...
| DSAF-25 Core Source ID becomes an alias post-pass | AC9 grep | Stale Source citation | Update `framework/dsaf-25.md` Source column to the new primary |
...
- **About the Part B ID-stabilisation footnote (per FR-CORE-001 §3b):** FR-CORE-001 left the Part B IDs as illustrative pending this FR. After this FR ships, the Part B IDs in `framework/dsaf-25.md` are *primary*, not illustrative. The illustrative caveat in FR-CORE-001 §3b is no longer accurate post-this-FR; FR-CORE-001 stays normative, but its §3b caveat may be deleted in a follow-up commit (or amended to "previously illustrative, stabilised at FR-CORE-003 land date 2026-05-17").
