...
**Canonical host:** `https://audit.cyberskill.world/`, per `internal/branding/domain-decision.md`.
...
|---|---|---|---|---|
| FR-BRAND-001 | PASS for ratified canonical-host scope | `internal/branding/domain-decision.md`; `internal/branding/brand-decoupling-domain-decision.md`; `landing/index.html`; `landing/card/index.html`; `landing/.well-known/security.txt`; `landing/robots.txt`; `landing/sitemap.xml`; `internal/ops/deploy-smoke-2026-05-18.md` | Live HTTP checks PASS for `/`, `/card`, `/.well-known/security.txt`, `/robots.txt`, `/sitemap.xml`; HSTS and CSP present; HTTP redirects to HTTPS with 308; no lead-capture form in landing HTML; `npm run verify` PASS after link-check hardening | Original `dsaf.dev` purchase / WHOIS / neutral-domain ACs superseded by `internal/branding/domain-decision.md` operator decision. Chrome visual check blocked by local macOS Computer Use permissions after 3 attempts. Pretty blog URLs currently 404 and are tracked for FR-DOCS-003 / A5, not BRAND-001. |
| FR-BRAND-001 strict reset | PASS with mocked private operations | `internal/branding/FR-BRAND-001-domain-contract.json`; `scripts/domain-contract-lib.mjs`; `scripts/check-domain-contract.mjs`; `scripts/check-domain-contract.test.mjs`; `outputs/_audit/domain-contract.json` | `node --test --experimental-test-coverage scripts/check-domain-contract.test.mjs` PASS 7/7 with 100.00% line coverage and 90.91% function coverage for `domain-contract-lib.mjs`; `npm run contract:domain` PASS with live summary `{"pass":26,"warn":0,"mocked":3,"fail":0}` and private summary `{"pass":103,"warn":0,"mocked":0,"fail":0}`; live curl showed HTTP/2 200, HSTS, CSP, security.txt, robots, sitemap, and HTTP 308 redirect | DNS CAA, DNS AAAA, HSTS preload status, registrar 2FA/vault, and mailbox forwarding are private/external gates covered by the mock operation contract. |
| FR-BRAND-002 | PASS | `internal/branding/handle-taxonomy.md`; `internal/branding/glossary.md`; `CONTRIBUTING.md`; README and landing taxonomy updates; `internal/branding/brand-audit-2026-05-18.md` | Repo banned-handle grep PASS on external surfaces; live deploy banned-handle checks PASS for `/` and `/card`; README has one long-name mention, 15 DSAF mentions, DSAF Criteria/Levels/Core present; `npm run verify` PASS | Live deploy still has repeated long-name metadata/body from the already-deployed version; the repo source is tightened and will apply on next Vercel deploy. Banned noun-handle forms are absent live. |
| FR-BRAND-002 strict reset | PASS | `internal/branding/FR-BRAND-002-taxonomy-contract.json`; `scripts/brand-taxonomy-contract-lib.mjs`; `scripts/check-brand-taxonomy-contract.mjs`; `scripts/check-brand-taxonomy-contract.test.mjs`; `outputs/_audit/brand-taxonomy-contract.json` | `node --test --experimental-test-coverage scripts/check-brand-taxonomy-contract.test.mjs` PASS 6/6 with 100.00% line coverage and 100.00% function coverage for `brand-taxonomy-contract-lib.mjs`; `npm run contract:brand-taxonomy` PASS with summary `{"pass":913,"warn":0,"fail":0,"ok":true}` over 99 files | Source-plan, FR specs, and taxonomy docs intentionally contain banned examples and are excluded from the executable external-surface contract. |
...
- Show HN payload file check → PASS: `internal/launch/show-hn-post.md` exists with title, URL field, body, founder first comment, schedule, T-15 checks, and post-launch updates.
- Response playbook check → PASS: `internal/launch/show-hn-response-playbook.md` exists with 4 SLA windows, 8 response patterns, 8+ anti-pattern rows, 7 kill-switches, and tracker handoff.
...
- Platform coverage → PASS: r/web_design, r/UXDesign, r/programming, Lobste.rs, daily.dev, and Designer News are all present in `internal/launch/cross-posts.md` and `internal/social/cross-posts.md`.
...
- Guardrails → PASS: no paid CTA in cross-post copy; moderation-removal no-repost rule and `internal/launch/post-hn-feedback.md` tracker handoff present.
- Posting dependency → BLOCKED: requires FR-LAUNCH-001 live HN URL, production blog/OG deploy, and platform account standing.

### FR-LAUNCH-003

- PH launch pack → PASS: `internal/launch/product-hunt-launch.md` exists with listing copy, schedule, gallery asset list, hunter shortlist, outreach template, maker comment, day-of runbook, and blockers.
...
- Canonical tracker → PASS: `internal/launch/headsup-outreach.md` exists with schedule, 10 named recipients, send protocol, response modes, and guardrails.
- Draft coverage → PASS: `internal/social/personal-outreach.md` contains 10 per-recipient email drafts for Brad Frost, Nathan Curtis, Sil Bormüller, Chris Strahl, Ben Callahan, Diana Mounter, Sarah Federman, Luke Murphy, Dan Mall, and Jina Anne.
- Shortlist status → PASS: `internal/branding/reviewer-shortlist.md` includes a distinct `Heads-up status (FR-LAUNCH-004)` column.
...
- Fix applied → `internal/social/personal-outreach.md` now includes canonical card URL, public repo URL, origin-story URL, long-name first mention, explicit no-upvote/no-amplification disclaimer, and optional 15-minute-call sentence in all 10 drafts.
...
- Targeted audit → PASS: `internal/launch/guest-post-pitches.md` exists and points to full copy-paste drafts in `internal/social/guest-post-pitches.md`.
...
- Initial targeted audit → FAIL: missing `internal/content/deep-dive-schedule.md`, missing public `landing/blog/deep-dives/index.md`, cadence said Monday instead of Tuesday 08:00 PT, and CODEOWNERS did not gate deep-dive surfaces.
- Fix applied → added the dated 12-week schedule, public deep-dive index, recursive/nested blog rendering, rendered Week 1 public article, favicon and code-block support, and CODEOWNERS gates for `internal/content/deep-dives/` plus `landing/blog/deep-dives/`.
...
- Initial targeted audit → FAIL: missing `internal/content/cross-publishing-playbook.md`, missing ready Week 1 dev.to/Medium/LinkedIn drafts, and `internal/social/cross-publishing-template.md` conflicted with the FR's same-content rule by requesting a condensed LinkedIn version.
...
- `internal/branding/reviewer-shortlist.md` row count → PASS, 10 named reviewers.
...
- `internal/social/reviewer-outreach-playbook-and-templates.md` top-3 drafts → PASS for Nathan, Sil, and Brad.
- `internal/branding/reviewer-outreach.md` guardrails → PASS for review-not-endorsement framing, no-upvote/no-repost language, and quote-approved state.
- `internal/branding/reviewer-consent-log.md` consent guard → PASS; no README quote may publish without a non-empty row.
- `npm run verify` → PASS.
...
- Fix applied → expanded `internal/governance/co-maintainer-charter.md`, `internal/governance/co-maintainer-shortlist.md`, `landing/blog/co-maintainer-announcement.md`, README, CONTRIBUTING, and `internal/branding/brand-decoupling-domain-decision.md`; added `internal/governance/co-maintainer-relationship-log.md`; regenerated rendered blog HTML.
...
- Fix applied → added `framework/bench/lite-benchmark-spec.md`, `landing/benchmark/index.html`, `landing/benchmark/results.html`, `landing/benchmark/results/index.html`, `landing/benchmark/privacy/index.html`; updated landing navigation, README, and legacy survey summary.
...
- Initial targeted audit → FAIL: only `guidelines/i18n/good-first-issues.md` existed; no translation guidelines, no GitHub issue template, no tracker, no native-speaker review gate, and no hreflang discipline.
- Fix applied → added `guidelines/i18n/translation-guidelines.md`, `guidelines/i18n/translation-tracker.md`, `.github/ISSUE_TEMPLATE/translation.md`, and `internal/social/translation-recruitment.md`; expanded `guidelines/i18n/good-first-issues.md`; updated `internal/social/README.md`.
...
- Mock payload artifact → PASS: `internal/social/FR-LAUNCH-006-social-payload.json` defines exact request body, expected `202` mock response, blocker, copy, schedule, and observability key for IDS Weekly, Pattern Pulse, Sidebar.io, and Smashing Newsletter.
- Contract test → PASS: `npm run contract:newsletter` returned `[newsletter-contract] PASS`, `submissions=4`, `mock_endpoint=POST /mock/newsletter-submissions`, and `tracking=internal/launch/newsletter-submissions.md`.
...
- Embed check → PASS: README embeds both visuals with dark-mode `<source>` variants; `guidelines/01-introduction.md`, `framework/07-maturity-tiers.md`, and `framework/dsaf-25.md` reference the canonical assets.
- Chrome headless screenshots at 1200x675 → PASS: both visuals render nonblank and recognisable.
- `npm run verify` → PASS.

### FR-BRAND-004

- `internal/branding/brand-decoupling-domain-decision.md` required-section check → PASS for Decision, Surfaces, Redirect posture, and Copy guardrails.
- `internal/branding/brand-decoupling-domain-decision.md` posture check → PASS: historical record only; no redirects installed; canonical URL is `https://audit.cyberskill.world/`.
...
- `framework/dsaf-25.md` row count → 25.
- Criteria ID resolution against `framework/03-criteria-part-a.md` and `framework/04-criteria-part-b.md` → 25/25 found.
- Category coverage → A1, A2, A3, A4, A5, A6, A7, A8, A9, A10, B1, B2, B3, B4, B5, B6, B7, B8, B9, B10 all represented.
- `framework/dsaf-25-card.md` item count → 25.
...
- `framework/02-framework.md`, `guidelines/06-fix-cycle.md`, and `framework/07-maturity-tiers.md` → describe explicit override or rollback behavior, not a hard no-downgrade rule.
- `npm run verify` → PASS.

### FR-CORE-003

- Criterion count check → PASS: `framework/03-criteria-part-a.md` has 63 rows, `framework/04-criteria-part-b.md` has 62 rows, total 125.
