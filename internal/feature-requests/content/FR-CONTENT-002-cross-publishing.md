...
new_files:
  - internal/content/cross-publishing-playbook.md
modified_files:
  - dsaf.dev/blog/deep-dives/_template.md   # add post-cross-publication "Discussion" section update guidance
allowed_tools:
  - "file_read/write internal/content/**, dsaf.dev/blog/deep-dives/**"
...
effort_hours: 3
sub_tasks:
  - "1. (30m) Author internal/content/cross-publishing-playbook.md per §3 — per-platform cross-publishing procedure + canonical-URL discipline + tracking format"
...
**2026-05-18 implementation note:** the cross-publishing pack is repo-shipped for the canonical host `audit.cyberskill.world`: `internal/content/cross-publishing-playbook.md` has platform procedures and a dated Week 1 posting schedule; `scripts/render-cross-publishing-drafts.mjs` generates full-content manual drafts; Week 1 dev.to, Medium, and LinkedIn drafts are present under `internal/content/deep-dives/cross-publishing/`.
...
### `internal/content/cross-publishing-playbook.md` — the canonical playbook
...
Feed back to `internal/content/deep-dive-schedule.md` (FR-CONTENT-001) for upcoming-cadence re-prioritisation.
...
1. **Playbook committed** — `internal/content/cross-publishing-playbook.md` exists with §3.1 per-platform procedures (dev.to + Medium + LinkedIn) + §3.2 dsaf.dev Discussion update + §3.3 per-platform anti-patterns + §3.4 tracking format + §3.5 engagement-pattern feedback + §3.6 substantive-edit sync.
2. **3 platforms enumerated** — `internal/content/cross-publishing-playbook.md` §3.1 has subsections for dev.to, Medium, LinkedIn.
...
11. **No auto-cross-publishing tools mentioned as approved** — `grep -ciE 'auto.cross.publish|RSS.import|auto.import' internal/content/cross-publishing-playbook.md` returns 0 (or returns only the anti-pattern mentions).
12. **Handle taxonomy compliance** — `grep -ciE '\b(the )?DSAF Framework\b|\bDSAF framework\b' internal/content/cross-publishing-playbook.md` returns 0; `grep -c '\bDSAF\b' internal/content/cross-publishing-playbook.md` ≥ 3.
13. **No 84.6 / L5 marketing** — `grep -ciE '84\.6|industry[- ]?leading|top tier|L5 Optimised' internal/content/cross-publishing-playbook.md` returns 0.
14. **No paid CTAs** — `grep -ciE 'Talk to a certified|Contact (us|sales|CyberSkill)|Schedule (a|your) demo|Book (a|your) call' internal/content/cross-publishing-playbook.md` returns 0.
...
test -f internal/content/cross-publishing-playbook.md
for platform in 'dev.to' 'Medium' 'LinkedIn'; do
  grep -qF "### ${platform}" internal/content/cross-publishing-playbook.md || echo "MISSING: ${platform}"
done

# AC3 — ≥ 7 steps per platform
for platform in 'dev.to' 'Medium' 'LinkedIn'; do
  awk -v p="### ${platform}" '$0 ~ p {flag=1; next} /^### / {flag=0} flag' internal/content/cross-publishing-playbook.md | \
    grep -cE '^[0-9]+\.'
  # expected: >= 7 per platform
done

# AC4 — canonical handling per platform
grep -q 'canonical_url' internal/content/cross-publishing-playbook.md   # dev.to
grep -q 'Edit story details' internal/content/cross-publishing-playbook.md   # Medium
grep -q 'Originally published at' internal/content/cross-publishing-playbook.md   # LinkedIn + others

# AC6 — staggered timing
grep -E 'T\+24h|T\+48h|T\+72h' internal/content/cross-publishing-playbook.md | wc -l
# expected: >= 3 (one per platform)

# AC7 — anti-patterns per platform
for platform in 'dev.to' 'Medium' 'LinkedIn'; do
  awk -v p="### ${platform} anti-patterns" '$0 ~ p {flag=1; next} /^### / {flag=0} flag' internal/content/cross-publishing-playbook.md | \
    grep -cE '^- \*\*'
  # expected: >= 4 per platform
done

# AC8 — tracking format
grep -q 'Deep-dive | dsaf.dev URL | dev.to URL | Medium URL | LinkedIn URL' internal/content/cross-publishing-playbook.md

# AC11 — no auto-tools approved
awk '/^### .* anti-patterns/{flag=0} /^## §3.6/{flag=0} /## /{flag=1} flag' internal/content/cross-publishing-playbook.md | \
...
grep -ciE '\b(the )?DSAF Framework\b|\bDSAF framework\b' internal/content/cross-publishing-playbook.md  # 0
grep -c '\bDSAF\b' internal/content/cross-publishing-playbook.md  # >= 3

# AC13 — no marketing claims
grep -ciE '84\.6|industry[- ]?leading|top tier|L5 Optimised' internal/content/cross-publishing-playbook.md  # 0

# AC14 — no paid CTAs
grep -ciE 'Talk to a certified|Contact (us|sales|CyberSkill)|Schedule (a|your) demo|Book (a|your) call' internal/content/cross-publishing-playbook.md  # 0
```
...
1. **(30m) Author `internal/content/cross-publishing-playbook.md`** per §3 — all 6 sub-sections.
