...
new_files:
  - internal/launch/headsup-outreach.md   # playbook + email template + tracking
modified_files:
  - internal/branding/reviewer-shortlist.md   # add a 'heads-up status' column distinct from 'outreach status'
allowed_tools:
  - "file_read/write internal/launch/**, internal/branding/**"
...
effort_hours: 5
sub_tasks:
  - "1. (30m) Author internal/launch/headsup-outreach.md per §3 — email template + per-reviewer personalisation guidance + tracking format"
  - "2. (15m) Cross-reference internal/branding/reviewer-shortlist.md (FR-GOV-001's shortlist); identify the 10 individuals; note any whose endorsement outreach already happened (no double-ask needed)"
...
**2026-05-18 implementation note:** the heads-up tracker is repo-shipped in `internal/launch/headsup-outreach.md`, with the ten named recipients, concrete May 2026 timing, response modes, and manual-send guardrails. Full per-recipient drafts are in `internal/social/personal-outreach.md`. The original T-7 window for a May 19 Show HN slot is already missed, so manual sending is blocked until the launch date rolls forward or Stephen accepts a late-send exception.

1. **MUST** send personal heads-up emails to 10 named individuals at T-7 days from FR-LAUNCH-001 Show HN posting date. The 10 are drawn from FR-GOV-001's shortlist (`internal/branding/reviewer-shortlist.md`) but the heads-up status is *independent* of endorsement-outreach status — a reviewer may have done endorsement outreach and now also gets heads-up (one personalised note covers both), OR a reviewer may have been skipped for endorsement outreach (e.g., not warm enough) but is still in scope for heads-up.
...
### `internal/launch/headsup-outreach.md` — the canonical heads-up doc
...
This file is the operations doc for FR-LAUNCH-004. The 10 named individuals from `internal/branding/reviewer-shortlist.md` get personalised heads-up emails at T-7 days from the FR-LAUNCH-001 Show HN posting date.
...
### `internal/branding/reviewer-shortlist.md` — column addition
...
1. **Heads-up doc committed** — `internal/launch/headsup-outreach.md` exists with §3.1 shortlist + §3.2 personalisation guidance + §3.3 email template + §3.4 response handling + §3.5 anti-patterns.
2. **10 recipients in §3.1** — `internal/launch/headsup-outreach.md` §3.1 table has at least 10 rows referencing the FR-GOV-001 shortlist.
...
6. **No vote-asking phrases** — `grep -ciE 'upvote|please upvote|please share|please amplify|please retweet|forward this' internal/launch/headsup-outreach.md` returns 0.
...
12. **Shortlist column added** — `internal/branding/reviewer-shortlist.md` has a `Heads-up status (FR-LAUNCH-004)` column.
13. **No paid CTA** — `grep -ciE 'audit\.cyberskill\.world|talk to a certified|book a call|schedule a demo' internal/launch/headsup-outreach.md` returns 0 (audit.cyberskill.world is acceptable only in the decoupling-disclosure context; the grep flags broader paid-funnel language).
14. **Handle taxonomy compliance** — `grep -ciE '\b(the )?DSAF Framework\b|\bDSAF framework\b' internal/launch/headsup-outreach.md` returns 0; `grep -c '\bDSAF\b' internal/launch/headsup-outreach.md` ≥ 8.
...
test -f internal/launch/headsup-outreach.md
awk '/^## §3.1/,/^## §3.2/' internal/launch/headsup-outreach.md | grep -cE '^\| [0-9]+ \|'
# expected: >= 10

# AC4 — email template length
awk '/^### Email template/,/^## §3.4/' internal/launch/headsup-outreach.md | \
  awk '/^```$/{flag=!flag; next} flag' | tr -d '\n' | wc -c
# expected: <= 2000

# AC6 — no vote-asking
grep -ciE 'upvote|please upvote|please share|please amplify|please retweet|forward this' internal/launch/headsup-outreach.md
# expected: 0

# AC7 — 3 response modes
for mode in 'no response' 'thanks, will look' 'substantive engagement'; do
  grep -qiF "${mode}" internal/launch/headsup-outreach.md || echo "MISSING mode: ${mode}"
done

# AC9 — anti-patterns
awk '/## §3.5 — Anti-patterns/,EOF' internal/launch/headsup-outreach.md | grep -cE '^- \*\*'
# expected: >= 6

# AC10 — 15-min call offer
grep -qi '15-min call\|15 min call' internal/launch/headsup-outreach.md

# AC11 — decoupling disclosure
grep -q 'open source.*one of several maintainers\|deliberately separated' internal/launch/headsup-outreach.md

# AC12 — shortlist column added
grep -q 'Heads-up status (FR-LAUNCH-004)' internal/branding/reviewer-shortlist.md

# AC13 — no broad paid CTA
grep -ciE 'audit\.cyberskill\.world|talk to a certified|book a call|schedule a demo' internal/launch/headsup-outreach.md
...
grep -ciE '\b(the )?DSAF Framework\b|\bDSAF framework\b' internal/launch/headsup-outreach.md  # 0
grep -c '\bDSAF\b' internal/launch/headsup-outreach.md  # >= 8
```
...
1. **(30m) Author `internal/launch/headsup-outreach.md`** per §3 — all 5 sub-sections.
2. **(15m) Cross-reference FR-GOV-001 shortlist.** Copy the 10 names from `internal/branding/reviewer-shortlist.md` into §3.1. Note any whose FR-GOV-001 endorsement outreach already happened (those get a consolidated note).
3. **(15m) Add shortlist column.** Patch `internal/branding/reviewer-shortlist.md` to add the `Heads-up status (FR-LAUNCH-004)` column.
