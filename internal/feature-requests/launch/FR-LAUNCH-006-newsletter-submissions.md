...
new_files:
  - internal/launch/newsletter-submissions.md   # per-newsletter submission procedure + tracking
  - internal/social/FR-LAUNCH-006-social-payload.json   # exact mock request/response contract for missing newsletter services
  - scripts/check-newsletter-contract.mjs   # dependency-free contract test + mock submitter
modified_files:
  - internal/social/newsletter-submissions.md
  - internal/social/README.md
  - README.md
  - package.json
...
effort_hours: 3
sub_tasks:
  - "1. (30m) Author internal/launch/newsletter-submissions.md per §3 — 4 per-newsletter procedures + submission timing + tracking format"
...
**2026-05-18 strict execution note:** repo-verifiable deliverables are shipped. Direct publishing is intentionally not attempted because the target newsletter services require human-owned accounts, login sessions, editor relationships, or a verified submission destination. The missing-service surface is isolated in `internal/social/FR-LAUNCH-006-social-payload.json`; `scripts/check-newsletter-contract.mjs` validates the exact request/response shape and mock submitter. Backlog state is `shipped + strict-audited + mocked-dependency`.
...
10. **MUST** track each submission in `internal/launch/newsletter-submissions.md` per §3 tracking format: deep-dive title + dsaf.dev URL + per-newsletter submission date + per-newsletter inclusion status (picked up Y/N + which issue date).
...
### `internal/launch/newsletter-submissions.md` — the canonical newsletter-submission doc
...
Per-deep-dive update at submission time (status: submitted) + at T+7d (status: included / not included). Maintain in `internal/launch/newsletter-submissions.md`.
...
Output: brief retrospective notes in `internal/launch/newsletter-submissions.md` 4-weekly-retrospective section; feed back to `internal/content/deep-dive-schedule.md` topic prioritisation.
...
1. **Newsletter doc committed** — `internal/launch/newsletter-submissions.md` exists per §3 with: per-newsletter procedure (4 newsletters), universal submission text template, per-deep-dive tracking, 4-weekly inclusion-pattern review process, anti-patterns.
2. **4 newsletters enumerated** — `internal/launch/newsletter-submissions.md` §3.1 has subsections for Into Design Systems Weekly, Pattern Pulse, Sidebar.io, Smashing Newsletter.
...
4. **No follow-up rule explicit** — `grep -q 'No follow-up\|forbidden per §1 #5' internal/launch/newsletter-submissions.md`.
5. **No paid-promotion rule explicit** — `grep -q 'paid promotion\|paid placement' internal/launch/newsletter-submissions.md` shows the rule is documented (in either main body or anti-patterns).
...
9. **FR-LAUNCH-001 launch context NOT in this FR's scope** — `internal/launch/newsletter-submissions.md` doesn't conflate FR-LAUNCH-001 launch submissions (the launch had its own newsletter outreach per FR-LAUNCH-001 + FR-LAUNCH-004 patterns); this FR's scope is FR-CONTENT-001 weekly deep-dive submissions.
10. **Handle taxonomy compliance** — `grep -ciE '\b(the )?DSAF Framework\b|\bDSAF framework\b' internal/launch/newsletter-submissions.md` returns 0; `grep -c '\bDSAF\b' internal/launch/newsletter-submissions.md` ≥ 4.
11. **No 84.6 / L5 marketing in submission text** — `grep -ciE '84\.6|industry[- ]?leading|top tier|L5 Optimised' internal/launch/newsletter-submissions.md` returns 0.
12. **No paid CTAs in submission text** — `grep -ciE 'Talk to a certified|Contact (us|sales|CyberSkill)|Schedule (a|your) demo|Book (a|your) call' internal/launch/newsletter-submissions.md` returns 0.
13. **Universal submission text template aligned with FR-CONTENT-002 cross-publishing canonical-URL discipline** — `grep -q 'canonical\|dsaf.dev URL' internal/launch/newsletter-submissions.md`.
...
test -f internal/launch/newsletter-submissions.md
for nl in 'Into Design Systems Weekly' 'Pattern Pulse' 'Sidebar.io' 'Smashing Newsletter'; do
  grep -qF "${nl}" internal/launch/newsletter-submissions.md || echo "MISSING: ${nl}"
done

# AC3 — per-newsletter procedure
for nl in 'Into Design Systems Weekly' 'Pattern Pulse' 'Sidebar.io' 'Smashing Newsletter'; do
  awk -v nl="### ${nl}" '$0 ~ nl {flag=1; next} /^### / {flag=0} flag' internal/launch/newsletter-submissions.md | \
    grep -E 'Submission method:|URL|submission text' || echo "INSUFFICIENT for ${nl}"
done

# AC4 — no follow-up rule
grep -q 'No follow-up\|no follow-up' internal/launch/newsletter-submissions.md

# AC5 — no paid-promotion rule
grep -qi 'paid promotion\|paid placement' internal/launch/newsletter-submissions.md

# AC6 — tracking format
grep -q '| Into Design Systems Weekly\|IDS Weekly' internal/launch/newsletter-submissions.md

# AC8 — ≥ 8 anti-patterns
awk '/## §3.5 — Anti-patterns/,EOF' internal/launch/newsletter-submissions.md | grep -cE '^- \*\*'
# expected: >= 8

# AC10 — handle taxonomy
grep -ciE '\b(the )?DSAF Framework\b|\bDSAF framework\b' internal/launch/newsletter-submissions.md  # 0
grep -c '\bDSAF\b' internal/launch/newsletter-submissions.md  # >= 4

# AC11 — no marketing claims in submission text
grep -ciE '84\.6|industry[- ]?leading|top tier|L5 Optimised' internal/launch/newsletter-submissions.md  # 0

# AC12 — no paid CTAs
grep -ciE 'Talk to a certified|Contact (us|sales|CyberSkill)|Schedule (a|your) demo|Book (a|your) call' internal/launch/newsletter-submissions.md  # 0

# AC13 — canonical URL discipline
grep -q 'canonical\|dsaf.dev URL' internal/launch/newsletter-submissions.md

# AC14 — co-maintainer inclusion
grep -q 'co-maintainer\|FR-GOV-002' internal/launch/newsletter-submissions.md
```
...
1. **(30m) Author `internal/launch/newsletter-submissions.md`** per §3 — all 5 sub-sections.
