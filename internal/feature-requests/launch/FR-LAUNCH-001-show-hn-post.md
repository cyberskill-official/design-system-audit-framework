...
new_files:
  - internal/launch/show-hn-post.md       # the exact post text + title + URL
  - internal/launch/show-hn-response-playbook.md   # 30/90/240-minute response SLA + comment templates + kill-switch
modified_files:
...
allowed_tools:
  - "file_read/write internal/launch/**, dsaf.dev/**, README.md"
...
effort_hours: 6
sub_tasks:
  - "1. (30m) Author internal/launch/show-hn-post.md per §3 — the exact title + post text + URL"
  - "2. (1h) Author internal/launch/show-hn-response-playbook.md per §3 — 30/90/240-minute SLA + comment-pattern templates + kill-switch condition"
  ...
  - "5. (over ~12-24 hours elapsed, ~4h founder-time) Monitor thread; respond per the playbook SLA; route critical feedback into a internal/launch/post-hn-feedback.md tracking file"
...
**2026-05-18 implementation note:** the operator payload and response playbook are repo-shipped in `internal/launch/show-hn-post.md` and `internal/launch/show-hn-response-playbook.md`, updated to the live public repo URL `github.com/cyberskill-official/design-system-audit-framework` and the ratified `audit.cyberskill.world` host. Manual HN posting remains blocked until FR-DOCS-002 has consented quotes or Stephen logs a launch exception, and until the production deploy serves `/blog/launch-2026` and `/assets/og/launch-2026-1200x630.png` with HTTP 200.
...
### `internal/launch/show-hn-post.md` — the canonical post
...
- Create `internal/launch/post-hn-feedback.md` tracking file (per FR-LAUNCH-001 §1 #10).
...
### `internal/launch/show-hn-response-playbook.md` — the response operations doc
...
> The README + blog post both cover this, but the short version: [SaaS competitor] is closed-source SaaS at $X/year; DSAF is OSS markdown you can fork. [SaaS competitor]'s rubric isn't publicly inspectable; DSAF's is at `framework/03-criteria-part-a.md`. The use cases overlap maybe 60% — if you need workflow + governance + SSO, [SaaS competitor] is right; if you need a rubric you can run quarterly without a vendor in the loop, DSAF is right. Genuine respect for [SaaS competitor].

### Pattern 5: someone asks "why should I care about this if I'm not a design-systems person?"

> Honest answer: probably you shouldn't. DSAF is for teams that have a design system AND want to assess its maturity — a small slice of the dev world. If you're not in that slice, the framework's interesting only as a methodology artefact (how you build criteria-graded maturity frameworks). The methodology bits are in `framework/02-framework.md` if you're curious; otherwise skip.
...
Create `internal/launch/post-hn-feedback.md` post-launch with the following template, and fill it as the thread progresses:
...
1. **show-hn-post.md committed** — `internal/launch/show-hn-post.md` exists with the title + body + URL field + founder's first comment + pre-launch checklist per §3.
2. **show-hn-response-playbook.md committed** — `internal/launch/show-hn-response-playbook.md` exists with: SLA table, ≥ 6 response patterns, ≥ 6 anti-patterns, ≥ 6 kill-switch conditions, tracking file template.
3. **Title verbatim** — `internal/launch/show-hn-post.md` contains the title `Show HN: DSAF – open-source maturity framework for design systems (L0–L5, 125 criteria, agent-native)` byte-identical (verified via `grep -F`).
...
8. **Pre-launch URL verification documented** — `internal/launch/show-hn-post.md` has the T-15-min curl verification block per §3.
9. **SLA matrix has 4 windows** — `internal/launch/show-hn-response-playbook.md` SLA table has rows for 0-4h, 4-12h, 12-24h, 24+h.
10. **6 response patterns documented** — `grep -cE '^### Pattern [0-9]:' internal/launch/show-hn-response-playbook.md` ≥ 6.
11. **6 anti-patterns documented** — `internal/launch/show-hn-response-playbook.md` "Response anti-patterns" table has ≥ 6 rows.
12. **6 kill-switch conditions** — `grep -cE '^[0-9]+\. \*\*' internal/launch/show-hn-response-playbook.md` (in the kill-switch section) ≥ 6.
13. **No vote-manipulation language** — `grep -ciE 'upvote|please upvote|ask.*upvote' internal/launch/show-hn-post.md internal/launch/show-hn-response-playbook.md` returns 0.
14. **Founder name + handle** — first comment contains the founder's name ("Stephen Cheng") and "CyberSkill" mention.
15. **Posting window documented** — `internal/launch/show-hn-post.md` frontmatter `scheduled_window` field is in Tuesday-Wednesday 8-10am PT window.
16. **Tracking file template present** — `internal/launch/show-hn-response-playbook.md` "Tracking file" section provides the template for `post-hn-feedback.md`.
17. **Dependency chain enforced** — `internal/launch/show-hn-post.md` pre-launch checklist includes verifying that FR-DOCS-001 + FR-DOCS-002 + FR-DOCS-003 are all at `accepted (10/10)` before posting.
...
test -f internal/launch/show-hn-post.md
test -f internal/launch/show-hn-response-playbook.md

# AC3 — title verbatim
grep -qF 'Show HN: DSAF – open-source maturity framework for design systems (L0–L5, 125 criteria, agent-native)' internal/launch/show-hn-post.md

# AC4 — body ≤ 1,000 chars
awk '/^## Show HN body/{flag=1; next} /^## /{flag=0} flag' internal/launch/show-hn-post.md | \
  tr -d '\n' | wc -c
# expected: ≤ 1000
...
grep -A 2 '^## Show HN URL field' internal/launch/show-hn-post.md | grep 'github.com/cyberskill-official/design-system-audit-framework'

# AC8 — pre-launch verification block
grep -q 'curl -sI' internal/launch/show-hn-post.md
grep -q 'dsaf.dev/card' internal/launch/show-hn-post.md

# AC9 — SLA matrix 4 windows
awk '/## Response SLA/,/## Response templates/' internal/launch/show-hn-response-playbook.md | \
  grep -cE '^\| [0-9]'
# expected: ≥ 4

# AC10 — 6 response patterns
grep -cE '^### Pattern [0-9]+:' internal/launch/show-hn-response-playbook.md
# expected: ≥ 6

# AC11 — 6 anti-patterns
awk '/## Response anti-patterns/,/## Kill-switch/' internal/launch/show-hn-response-playbook.md | \
  grep -cE '^\| "'
# expected: ≥ 6

# AC12 — 6 kill-switch conditions
awk '/## Kill-switch conditions/,/## Tracking file/' internal/launch/show-hn-response-playbook.md | \
  grep -cE '^[0-9]+\. \*\*'
# expected: ≥ 6

# AC13 — no vote-manipulation language
grep -ciE 'upvote|please upvote|ask.*upvote' internal/launch/show-hn-post.md internal/launch/show-hn-response-playbook.md
# expected: 0

# AC14 — founder name + handle
grep -q 'Stephen Cheng' internal/launch/show-hn-post.md
grep -q 'CyberSkill' internal/launch/show-hn-post.md

# AC15 — posting window
grep -E 'Tuesday|Wednesday' internal/launch/show-hn-post.md | grep -E '8|9|10' | grep -i 'pt\|pacific'

# AC16 — tracking file template
grep -q '## Tracking file' internal/launch/show-hn-response-playbook.md
grep -q 'internal/launch/post-hn-feedback.md' internal/launch/show-hn-response-playbook.md

# AC17 — dependency chain in pre-launch checklist
grep -E 'FR-DOCS-001|FR-DOCS-002|FR-DOCS-003' internal/launch/show-hn-post.md
```
...
1. **(30m) Author `internal/launch/show-hn-post.md`** per §3 — title, body, URL field, founder's first comment, pre-launch checklist.
2. **(1h) Author `internal/launch/show-hn-response-playbook.md`** per §3 — SLA, response patterns, anti-patterns, kill-switch conditions, tracking template.
...
6. **(monitoring window — over 12 hours elapsed, ~3h founder-time)** Monitor thread. Respond per the SLA. Track each substantive comment in `internal/launch/post-hn-feedback.md`. If kill-switch fires, pause and consult the playbook.
