...
new_files:
  ...
  - framework/bench/lite-benchmark-spec.md   # spec doc: questions, data model, anonymisation contract
modified_files:
...
allowed_tools:
  - "file_read/write dsaf.dev/**, framework/bench/**, README.md"
...
effort_hours: 8
sub_tasks:
  - "1. (1h) Author framework/bench/lite-benchmark-spec.md per §3 — questions (25 — one per DSAF-25 Core criterion + 4 demographic-anonymous-bucket questions), data model, anonymisation contract"
...
### `framework/bench/lite-benchmark-spec.md` — the canonical spec
...
For each of the 25 DSAF-25 Core criteria (per `framework/dsaf-25.md`), the respondent selects one of 6 options:
...
<ul>
  <li>Read the full 125-criterion rubric: <a href="https://github.com/cyberskill-official/design-system-audit-framework/blob/main/framework/03-criteria-part-a.md">Part A — System</a> + <a href="https://github.com/cyberskill-official/design-system-audit-framework/blob/main/framework/04-criteria-part-b.md">Part B — UX</a></li>
...
1. **Spec doc committed** — `framework/bench/lite-benchmark-spec.md` exists per §3 with §3.1 Questions + §3.2 Data model + §3.3 Anonymisation contract + §3.4 Results page spec + §3.5 Cap-rule disclosure + §3.6 Privacy policy.
2. **29 questions specified** — `framework/bench/lite-benchmark-spec.md` §3.1 has 25 DSAF-25 Core criterion questions + 4 demographic-bucket questions + 1 optional free-text field.
...
4. **Form vendor named** — `framework/bench/lite-benchmark-spec.md` + `dsaf.dev/benchmark/privacy.md` name a GDPR-compliant vendor (Tally per recommendation; Typeform Pro / Formspree acceptable).
...
7. **Low-N guard ≥ 30 documented** — `framework/bench/lite-benchmark-spec.md` §3.3 + §3.4 both state the "≥ 30 responses required" rule.
...
9. **No PII required** — `framework/bench/lite-benchmark-spec.md` §3.1 + form configuration confirm no required PII fields.
...
11. **No paid-funnel CTAs** — `grep -ciE 'Talk to a certified|Contact (us|sales|CyberSkill)|Schedule (a|your) demo|Book (a|your) call' dsaf.dev/benchmark/*.html framework/bench/lite-benchmark-spec.md` returns 0.
...
16. **Handle taxonomy compliance** — `grep -ciE '\b(the )?DSAF Framework\b|\bDSAF framework\b' dsaf.dev/benchmark/*.html framework/bench/lite-benchmark-spec.md` returns 0.
...
```bash
# AC1 — spec doc
test -f framework/bench/lite-benchmark-spec.md
for section in '## §3.1 — Questions' '## §3.2 — Data model' '## §3.3 — Anonymisation contract' '## §3.4 — Results page' '## §3.5 — Cap-rule disclosure' '## §3.6 — GDPR-compliant privacy policy'; do
  grep -qF "${section}" framework/bench/lite-benchmark-spec.md || echo "MISSING: ${section}"
done

# AC2 — 29 questions
grep -cE '^\*\*Q[0-9]+' framework/bench/lite-benchmark-spec.md  # >= 4 (demographic + free-text); criterion Q1-Q25 referenced separately
grep -c 'DSAF-25 Core' framework/bench/lite-benchmark-spec.md   # criterion-count anchor
...
grep -q 'Tally\|Typeform\|Formspree' framework/bench/lite-benchmark-spec.md
...
# AC7 — low-N guard
grep -q '≥ 30\|N ≥ 30' framework/bench/lite-benchmark-spec.md
...
# AC11 — no paid CTAs
grep -ciE 'Talk to a certified|Contact (us|sales|CyberSkill)|Schedule (a|your) demo|Book (a|your) call' dsaf.dev/benchmark/index.html dsaf.dev/benchmark/results.html framework/bench/lite-benchmark-spec.md
# expected: 0
...
grep -ciE '\b(the )?DSAF Framework\b|\bDSAF framework\b' dsaf.dev/benchmark/index.html dsaf.dev/benchmark/results.html framework/bench/lite-benchmark-spec.md
# expected: 0
```
...
1. **(1h) Author `framework/bench/lite-benchmark-spec.md`** per §3 — all sub-sections.
