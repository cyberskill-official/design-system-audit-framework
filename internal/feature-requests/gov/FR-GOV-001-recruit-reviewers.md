...
new_files:
  - internal/branding/reviewer-outreach.md
  - internal/branding/reviewer-shortlist.md
  - internal/governance/FR-GOV-001-reviewer-contract.json
...
effort_hours: 6
sub_tasks:
  - "1. (30m) Author internal/branding/reviewer-shortlist.md per §3 — the named 10-person shortlist from the plan, ranked by warmth + outreach order"
  - "2. (30m) Author internal/branding/reviewer-outreach.md per §3 — the outreach playbook + consent letter template"
...
9. **MUST** track outreach status in `internal/branding/reviewer-shortlist.md` per the table format in §3: name / role / affiliation / outreach date / response date / response category (not contacted, contacted, replied-positive, replied-negative, no-response, quote-approved). The tracking file is the audit trail.
...
### `internal/branding/reviewer-shortlist.md` (NEW) — body shape
...
### `internal/branding/reviewer-outreach.md` (NEW) — body shape
...
Status update in `internal/branding/reviewer-shortlist.md`: `replied-negative`. No public mention. Memory update: `<Name> declined gracefully 2026-MM-DD; revisit at P2 if FR-GOV-002 outreach reaches them.`
...
1. **Shortlist committed** — `internal/branding/reviewer-shortlist.md` exists with the 10-row table format from §3 (ranked + status-tracked).
2. **Outreach playbook committed** — `internal/branding/reviewer-outreach.md` exists with: outreach email template, consent letter template, decline-handling for the three categories, response timeline, privacy section, anti-patterns.
3. **Shortlist populated** — at least 5 of the 10 rows in `internal/branding/reviewer-shortlist.md` have a name + role + warmth-score + outreach-status filled in (the other 5 may stay as placeholders if the operator hasn't ranked them yet).
4. **Outreach started for top-3 warmest** — `internal/branding/reviewer-shortlist.md` shows `contacted` status for the top-3-by-warmth rows. PR description includes the dates each was contacted.
...
6. **2+ quotes committed to a holding file** — quotes are stored in a holding file (NOT yet in README — that's FR-DOCS-002's job). Holding file path: `internal/branding/reviewer-quotes-pending.md` (gitignored or repo-private until FR-DOCS-002 commits them to README).
7. **Consent letters logged** — for every quote in the holding file, a corresponding consent letter (in `internal/branding/reviewer-consent-log.md` or in the founder's personal email folder) confirms the reviewer's explicit written approval. PR description references the consent-log file (which may be repo-private).
8. **Quotes within 280-char limit** — every quote committed for publication is ≤ 280 chars. `awk '/^> "/' internal/branding/reviewer-quotes-pending.md | awk '{ if (length > 280) print "TOO LONG"; }'` returns no output.
9. **No reviewer published without consent** — `grep -c '<endorsement quote' README.md` ≥ 2 only AFTER FR-DOCS-002 ships AND `internal/branding/reviewer-consent-log.md` documents each quote's approval. AC9 is verified at FR-DOCS-002 land time, not at this FR's land time.
...
11. **No paid compensation in outreach** — `grep -ciE 'pay\|compensation\|honorarium\|fee' internal/branding/reviewer-outreach.md` returns 0 outside the "anti-patterns" or "decline-handling" sections.
12. **Geography-headwind context transparent** — `internal/branding/reviewer-outreach.md` outreach email template mentions "Vietnam-based" or "geography-headwind" framing explicitly.
13. **3 decline modes documented** — `internal/branding/reviewer-outreach.md` "Decline-handling" section has subsections for `(i) "No time / no fit"`, `(ii) "Feedback but no quote"`, `(iii) "Quote but private"` per §3.
...
test -f internal/branding/reviewer-shortlist.md
grep -q '## Shortlist' internal/branding/reviewer-shortlist.md

# AC2 — outreach playbook committed
test -f internal/branding/reviewer-outreach.md
for section in '## The ask' '## Outreach email template' '## Consent letter template' '## Decline-handling' '## Response timeline'; do
  grep -qF "${section}" internal/branding/reviewer-outreach.md || echo "MISSING: ${section}"
done

# AC3 — shortlist populated (5+ rows)
populated_rows=$(awk -F '|' '/^\| [0-9]+ \|/ { if ($3 !~ /<re-ranked-name/) print }' internal/branding/reviewer-shortlist.md | wc -l)
[ "${populated_rows}" -ge 5 ] || echo "FAIL AC3: only ${populated_rows} rows populated"

# AC4 — top-3 contacted
contacted=$(grep -c 'contacted\|replied-positive\|replied-negative\|quote-approved\|no-response' internal/branding/reviewer-shortlist.md)
...
positive=$(grep -cE 'replied-positive|quote-approved' internal/branding/reviewer-shortlist.md)
...
if [ -f internal/branding/reviewer-quotes-pending.md ]; then
  awk '/^> "/ { if (length > 280) print "TOO LONG: " $0 }' internal/branding/reviewer-quotes-pending.md
fi

# AC11 — no paid compensation mention (outside anti-patterns / decline-handling)
grep -niE 'pay|compensation|honorarium|fee' internal/branding/reviewer-outreach.md | \
  grep -v 'Anti-patterns\|Decline-handling' | \
  grep -v '^[0-9]*:Subject:'
# expected: empty

# AC12 — geography context
grep -qi 'vietnam\|geography' internal/branding/reviewer-outreach.md

# AC13 — three decline modes documented
for mode in 'No time' 'Feedback but no quote' 'Quote but private'; do
  grep -qF "${mode}" internal/branding/reviewer-outreach.md || echo "MISSING decline mode: ${mode}"
done
```
...
1. **(30m) Author `internal/branding/reviewer-shortlist.md`.** Use the §3 seed-set as starting candidates. Re-rank 5+ rows by warmth using the §3 criteria. Commit.
2. **(30m) Author `internal/branding/reviewer-outreach.md`.** Copy the §3 body verbatim. Commit.
...
6. **(15m per approved quote)** When a reviewer's consent comes back, commit the quote to `internal/branding/reviewer-quotes-pending.md` (the holding file). Update MEMORY.md.
...
Quotes in holding: 2 (`internal/branding/reviewer-quotes-pending.md`). Consent letters: 2 (sent; awaiting one approval; one approved + logged).
