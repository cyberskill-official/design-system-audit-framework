...
- CI evidence: `outputs/_audit/coverage.json`, `bundle-size.json`, `doc-freshness.json`, `apca-contrast.json`
- Previous audit: `outputs/_audit/audit-report-{parent_audit}.md` (if any)
...
register_row_added: true  # appended to outputs/_audit/_history.md
```

Then append the row to your audit history register (typically `_audit/_history.md` or `outputs/_audit/_history.md`, depending on where you scaffolded the audit folder) per the framework's sign-off rule.

---

## §10 Criteria scores (machine-readable)

> 125 rows. Stable column order: `id | category | criterion | weight | pre_score | post_score | confidence | regression_tag | citations | notes`. Agents parse this directly. Fill from `framework/03-criteria-part-a.md` and `framework/04-criteria-part-b.md`.
