...
For maximal audits, the enterprise overlay in [`framework/bench/maximal-enterprise-benchmark.md`](../framework/bench/maximal-enterprise-benchmark.md) can be reported beside the DSAF Level. It does not create L6 or replace L0-L5. It answers a different question: which large-enterprise requirements are evidenced, which AUTO rows can be improved through source/artifact work, and which MANUAL rows require human proof before audited claims.
...
DSAF's no-silent-regression rule (per [`01-framework-overview.md`](./01-framework-overview.md) §4) means every regression in a signed audit is surfaced with a cause, tag, and approval path. **The DSAF Level might still change** if you cross a threshold downward via a DYNAMIC rubric tightening.
...
*Continue to [`08-improvement-plan.md`](../guidelines/08-improvement-plan.md) for how to turn the audit into a phased plan.*
