...
- **If rejected at all three:** the rejection notes (if any) are useful editorial signal. Update `internal/launch/guest-post-pitches.md` with the rejection reasons + decision on whether to re-pitch elsewhere (Pattern Pulse, dev.to long-form, Increment, etc.).
