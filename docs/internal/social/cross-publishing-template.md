...
Update `internal/content/cross-publishing.md` per post:

| Canonical | dev.to | Medium | LinkedIn | Inclusions |
|---|---|---|---|---:|
| `<slug>` | `<dev.to URL>` | `<Medium URL>` | `<LinkedIn URL>` | 0 |

The "Inclusions" column counts newsletter pickups (see `internal/social/newsletter-submissions.md`). Update at T+14d per post.
