...
**Cadence:** one post per Tuesday at 08:00 PT / 22:00 ICT for 12 weeks starting four weeks after launch. **Format:** CEA (Context → Example → Anti-pattern), 1,400–2,000 words per post, one criterion per post. **Canonical home:** `https://audit.cyberskill.world/blog/<slug>`.
...
7. Generate the cross-publish drafts using `internal/social/cross-publishing-template.md` and save as `week-XX-<...>--devto.md`, `--medium.md`, `--linkedin.md` in the same folder.

## Style invariants (apply to every post)

- **One criterion per post.** Don't bundle.
- **The criterion is quoted byte-identical** from `framework/03-criteria-part-a.md` / `framework/04-criteria-part-b.md`.
...
| dev.to | T+24h | Per `internal/social/cross-publishing-template.md` |
| Medium | T+48h | Per same template |
| LinkedIn long-form | T+72h | Per same template (condensed) |
| Newsletter submissions | T+24h | Per `internal/social/newsletter-submissions.md` |
