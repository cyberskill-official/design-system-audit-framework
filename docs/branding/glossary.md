...
- **No-silent-regression rule** — the integrity rule ratified by FR-CORE-002. A FIXED criterion can regress, but only with an explicit override comment naming the cause; silent regression is detected and refused. Long-form: [`docs/regression-policy.md`](../regression-policy.md).
