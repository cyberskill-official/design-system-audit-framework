# Reviewer outreach — top 3 from shortlist
...
The ask is **review, not endorsement** per `docs/branding/reviewer-outreach.md`. Quotes are usable only after the consent letter is co-signed.
...
## Consent letter (send to anyone who replies "interested in giving a quote")
...
- You agree to the surfaces: README.md, the audit.cyberskill.world launch page, and the Show HN launch comment (if you authorise that surface specifically)
- The duration is 12 months minimum
- You can retract within 7 days of a written request
...
When the reply lands, paste the exact text + the reply timestamp + the reviewer email into `docs/branding/reviewer-consent-log.md` per FR-GOV-001 §1.
...
Update `docs/branding/reviewer-shortlist.md` Status column per reviewer: `not-contacted` → `contacted` → `replied-positive` / `feedback-no-quote` / `quote-approved` / `quote-published` / `declined` / `no-response`.

*End of reviewer outreach drafts.*
