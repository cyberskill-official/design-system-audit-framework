...
Pre-revision §3 `CRITERION_METADATA` const listed ~9 criteria with the comment "16 more entries needed." Each new criterion (FR-CORE-003 dedup, P6 RFCs) requires manual updates + version bump. The maintenance burden isn't trivial. **Resolved:** §11 implementation note "About the criterion metadata" explicit — current approach is manual maintenance; future automation generates `CRITERION_METADATA` from `docs/03-criteria-part-a.md` + `docs/04-criteria-part-b.md` at addon build time (deferred to a follow-on FR). Pattern: §3.6 rule 18 (maintenance burden + future automation path documented).
