...
new_files:
  ...
  - docs/content/deep-dive-schedule.md      # 12-week initial schedule + topic backlog
modified_files:
  - dsaf.dev/blog/index.md   # parent blog index now lists deep-dives section
allowed_tools:
  - "file_read/write dsaf.dev/**, docs/content/**"
...
sub_tasks:
  ...
  - "2. (1h) Author docs/content/deep-dive-schedule.md per §3 — 12-week initial schedule with criterion + priority rationale per week"
...
11. **MUST** log skipped weeks in `docs/content/deep-dive-schedule.md` with reason. Silent skips degrade the cadence signal; explicit "no post this week (reason)" entries preserve transparency. Acceptable skip reasons: founder-illness, framework-crisis (launch-week fire), or rare scheduled events (conference-week). > 2 skips in any 12-week window triggers the §10 failure-mode "cadence breakdown" path.
...
14. **MUST NOT** publish a deep-dive on a criterion that doesn't exist in DSAF-125 (the current rubric). Per FR-CORE-001's verbatim-quote rule: if a deep-dive's intended criterion is a *proposed* addition (RFC pending per FR-GOV-003 P6), the deep-dive ships only after the criterion lands in `docs/03-criteria-part-a.md` or `docs/04-criteria-part-b.md`. Pre-RFC deep-dives are RFC drafts, not weekly cadence posts.
...
> **[Criterion ID] — [Verbatim criterion name from docs/03-criteria-part-a.md or docs/04-criteria-part-b.md]**
>
...
- Document decision in `docs/content/deep-dive-schedule.md` updated section + MEMORY.md per cadence-state continuity.
```
...
2. **Schedule committed** — `docs/content/deep-dive-schedule.md` exists with 12-week initial schedule + topic backlog + skipped-week log + 4-weekly review process + Week-12 retrospective decision section.
...
8. **12 weeks scheduled** — `docs/content/deep-dive-schedule.md` 12-week schedule has 12 rows (W1-W12) with criterion ID + brief + priority rationale per row (placeholders OK pre-launch; filled at week-1 publication time).
9. **Topic backlog ≥ 12 entries** — `docs/content/deep-dive-schedule.md` topic backlog has ≥ 12 rows for post-W12 candidates.
...
13. **Handle taxonomy compliance** — `grep -ciE '\b(the )?DSAF Framework\b|\bDSAF framework\b' dsaf.dev/blog/deep-dives/_template.md docs/content/deep-dive-schedule.md` returns 0; `grep -c '\bDSAF\b' dsaf.dev/blog/deep-dives/_template.md` ≥ 4.
14. **No 84.6 / L5 marketing** — `grep -ciE '84\.6|industry[- ]?leading|top tier|L5 Optimised' dsaf.dev/blog/deep-dives/_template.md docs/content/deep-dive-schedule.md` returns 0.
...
awk '/## 12-week initial schedule/,/## Topic backlog/' docs/content/deep-dive-schedule.md | \
  grep -cE '^\| W[0-9]+'
# expected: 12

# AC9 — topic backlog ≥ 12
awk '/## Topic backlog/,/## Skipped-week log/' docs/content/deep-dive-schedule.md | \
  grep -cE '^\| [0-9]+'
# expected: >= 12

# AC13 — handle taxonomy
grep -ciE '\b(the )?DSAF Framework\b|\bDSAF framework\b' dsaf.dev/blog/deep-dives/_template.md docs/content/deep-dive-schedule.md  # 0
grep -c '\bDSAF\b' dsaf.dev/blog/deep-dives/_template.md  # >= 4

# AC14 — no 84.6 / L5 marketing
grep -ciE '84\.6|industry[- ]?leading|top tier|L5 Optimised' dsaf.dev/blog/deep-dives/_template.md docs/content/deep-dive-schedule.md  # 0
...
2. **(1h) Author `docs/content/deep-dive-schedule.md`** per §3 — 12-week schedule + topic backlog + skipped-week log + review/retrospective process.
