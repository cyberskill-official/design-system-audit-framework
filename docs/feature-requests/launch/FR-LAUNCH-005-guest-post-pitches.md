...
new_files:
  - docs/launch/guest-post-pitches.md
modified_files: []
allowed_tools:
...
effort_hours: 4
sub_tasks:
  - "1. (30m) Author docs/launch/guest-post-pitches.md per §3 — pitch templates per publication + topic shortlist + editorial-contact research"
...
### `docs/launch/guest-post-pitches.md` — the canonical pitch doc
...
1. **Pitch doc committed** — `docs/launch/guest-post-pitches.md` exists with §3.1 editorial research + §3.2 tone calibration + §3.3 topic shortlist + §3.4 pitch template + §3.5 response handling + §3.6 tracking format + §3.7 drafting discipline + §3.8 anti-patterns.
2. **3 publications enumerated** — `docs/launch/guest-post-pitches.md` §3.1 table has rows for Smashing Magazine, CSS-Tricks, A List Apart.
...
10. **No third-party SEO mention** — `grep -ciE 'paid placement|guaranteed publication|seo service' docs/launch/guest-post-pitches.md` returns 0 (except in the anti-patterns).
11. **Decoupling-disclosure framing** — `grep -q 'audit.cyberskill.world' docs/launch/guest-post-pitches.md` returns 0 OR the references are explicitly in disclosure / "About the author" context, not as CTAs.
12. **Handle taxonomy compliance** — `grep -ciE '\b(the )?DSAF Framework\b|\bDSAF framework\b' docs/launch/guest-post-pitches.md` returns 0; `grep -c '\bDSAF\b' docs/launch/guest-post-pitches.md` ≥ 8.
...
test -f docs/launch/guest-post-pitches.md
for section in '## §3.1 — Publication' '## §3.2 — Per-publication tone' '## §3.3 — Topic shortlist' '## §3.4 — Pitch email template' '## §3.5 — Response handling' '## §3.6 — Tracking format' '## §3.7 — Article-drafting discipline' '## §3.8 — Anti-patterns'; do
  grep -qF "${section}" docs/launch/guest-post-pitches.md || echo "MISSING: ${section}"
done

# AC2 — 3 publications
for pub in 'Smashing' 'CSS-Tricks' 'A List Apart'; do
  grep -qF "${pub}" docs/launch/guest-post-pitches.md || echo "MISSING: ${pub}"
done

# AC3 — 3 topic sections
grep -cE '^### Topic [ABC]' docs/launch/guest-post-pitches.md
# expected: 3

# AC4 — topics differ from launch blog post (manual check; the working titles are all different)
grep -i 'here.s what we got wrong\|origin story' docs/launch/guest-post-pitches.md | grep -v 'launch blog post'
# expected: empty (no §3.3 topic uses the launch-blog phrasing as a title)

# AC5 — pitch template character count
awk '/^### Pitch email template/,/^## §3.5/' docs/launch/guest-post-pitches.md | \
...
for mode in 'acceptance' 'revision request' 'rejection'; do
  grep -qiF "${mode}" docs/launch/guest-post-pitches.md || echo "MISSING mode: ${mode}"
done

# AC10 — no SEO-service mention (outside anti-patterns)
grep -niE 'paid placement|guaranteed publication|seo service' docs/launch/guest-post-pitches.md | grep -v 'Anti-patterns\|forbidden'
# expected: empty

# AC11 — no audit.cyberskill.world CTA (manual check; reviewer confirms any occurrences are disclosure not CTA)
grep -n 'audit.cyberskill.world' docs/launch/guest-post-pitches.md
# expected: empty OR each occurrence in disclosure context

# AC12 — handle taxonomy
grep -ciE '\b(the )?DSAF Framework\b|\bDSAF framework\b' docs/launch/guest-post-pitches.md  # 0
grep -c '\bDSAF\b' docs/launch/guest-post-pitches.md  # >= 8

# AC14 — 8 anti-patterns
awk '/## §3.8 — Anti-patterns/,EOF' docs/launch/guest-post-pitches.md | grep -cE '^- \*\*'
# expected: >= 8
```
...
1. **(30m) Author `docs/launch/guest-post-pitches.md`** per §3 — all 8 sub-sections.
