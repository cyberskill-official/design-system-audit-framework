...
new_files:
  - docs/launch/product-hunt-launch.md
modified_files:
  - docs/launch/post-hn-feedback.md   # extended post-launch with PH metrics + comment patterns
allowed_tools:
...
effort_hours: 4
sub_tasks:
  - "1. (30m) Author docs/launch/product-hunt-launch.md per §3 — PH listing copy + hunter recruit shortlist + day-of run book"
  ...
  - "6. (T+24h post-PH) Update docs/launch/post-hn-feedback.md with PH outcome (upvotes, comments, makers-of-the-day rank if any)"
...
### `docs/launch/product-hunt-launch.md` — the canonical PH launch doc
...
| T+24h | Update docs/launch/post-hn-feedback.md with PH outcome (upvotes, comments, badge if any) |
...
1. **PH launch doc committed** — `docs/launch/product-hunt-launch.md` exists with §3.1 listing copy + §3.2 image assets + §3.3 founder first comment + §3.4 hunter shortlist + §3.5 outreach template + §3.6 day-of run book + §3.7 pre-launch verification + §3.8 tracking extension.
...
13. **Handle taxonomy compliance** — `grep -ciE 'DSAF Framework|DSAF framework' docs/launch/product-hunt-launch.md` returns 0; `grep -c '\bDSAF\b' docs/launch/product-hunt-launch.md` ≥ 5.
14. **No 84.6% or L5 marketing** — `grep -ciE '84\.6|industry[- ]?leading|top tier|L5 Optimised' docs/launch/product-hunt-launch.md` returns 0.
15. **No vote-manipulation language** — `grep -ciE 'upvote|please upvote' docs/launch/product-hunt-launch.md` returns 0.
...
test -f docs/launch/product-hunt-launch.md
for section in '## §3.1 — Listing copy' '## §3.2 — Image assets' '## §3.3 — Founder' '## §3.4 — Hunter recruit' '## §3.5 — Hunter outreach' '## §3.6 — Day-of run book' '## §3.7 — Pre-launch verification' '## §3.8 — Tracking file extension'; do
  grep -qF "${section}" docs/launch/product-hunt-launch.md || echo "MISSING: ${section}"
done

# AC2, AC3, AC4 — character-count caps
title_chars=$(awk '/^### Title/{flag=1;next}/^###/{flag=0}flag' docs/launch/product-hunt-launch.md | grep -v '^```' | grep -v '^$' | tr -d '\n' | wc -c)
[ "${title_chars}" -le 60 ] || echo "FAIL AC2: title ${title_chars} chars"

tagline_chars=$(awk '/^### Tagline/{flag=1;next}/^###/{flag=0}flag' docs/launch/product-hunt-launch.md | grep -v '^```' | grep -v '^$' | tr -d '\n' | wc -c)
[ "${tagline_chars}" -le 60 ] || echo "FAIL AC3: tagline ${tagline_chars} chars"

desc_chars=$(awk '/^### Description/{flag=1;next}/^###/{flag=0}flag' docs/launch/product-hunt-launch.md | grep -v '^```' | grep -v '^$' | tr -d '\n' | wc -c)
...
awk '/^## §3.4 — Hunter recruit/,/^## §3.5/' docs/launch/product-hunt-launch.md | grep -cE '^\| [0-9]'
# expected: ≥ 3

# AC10 — run book ≥ 8 timing entries
awk '/^## §3.6 — Day-of run book/,/^## §3.7/' docs/launch/product-hunt-launch.md | grep -cE '^\| T'
# expected: ≥ 8

# AC11 — pre-launch verification has URL + asset checks
grep -q 'curl -sI' docs/launch/product-hunt-launch.md
grep -q 'wc -c' docs/launch/product-hunt-launch.md

# AC13 — handle taxonomy
grep -ciE '\b(the )?DSAF Framework\b|\bDSAF framework\b' docs/launch/product-hunt-launch.md  # 0
grep -c '\bDSAF\b' docs/launch/product-hunt-launch.md  # >= 5

# AC14 — no 84.6 / L5 marketing
grep -ciE '84\.6|industry[- ]?leading|top tier|L5 Optimised' docs/launch/product-hunt-launch.md  # 0

# AC15 — no vote-manipulation
grep -ciE 'upvote|please upvote' docs/launch/product-hunt-launch.md  # 0

# AC16 — decoupling disclosure in first comment
awk '/^## §3.3/,/^## §3.4/' docs/launch/product-hunt-launch.md | grep -q 'audit.cyberskill.world.*separate site'
```
...
1. **(30m) Author `docs/launch/product-hunt-launch.md`** per §3 — all 8 sub-sections.
