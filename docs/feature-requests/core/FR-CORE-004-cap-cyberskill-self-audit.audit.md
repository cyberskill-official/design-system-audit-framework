...
Pre-revision AC11 required the cap rationale in "README first 200 words" but FR-DOCS-001 (README rewrite) hasn't shipped yet — at this FR's land time, the README first paragraph may not have stabilised. **Resolved:** §3 README patch section made explicitly *conditional* on existing phrasing, with a no-op clause when none of the banned phrasings are present. AC11 amended to verify the rationale in *either* `docs/01-introduction.md` (which this FR amends directly) *or* the README (when FR-DOCS-001 ships). The dependency ordering between FR-CORE-004 and FR-DOCS-001 now makes either-first-shippable feasible. Pattern: §8.6c (§1 MUST clarity — was implicit, now explicit).
