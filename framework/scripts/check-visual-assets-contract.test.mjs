...
function fixtureRepo() {
  ...
  writeFileSync(join(root, "framework/assets/dsaf-visual-design-spec.md"), Object.values(payload.required_strings["framework/assets/dsaf-visual-design-spec.md"]).join("\n"));
  writeFileSync(join(root, "README.md"), payload.required_strings["README.md"].join("\n"));
  writeFileSync(join(root, "guidelines/01-introduction.md"), payload.required_strings["guidelines/01-introduction.md"].join("\n"));
  writeFileSync(join(root, "framework/07-maturity-tiers.md"), payload.required_strings["framework/07-maturity-tiers.md"].join("\n"));
  writeFileSync(join(root, "framework/dsaf-25.md"), payload.required_strings["framework/dsaf-25.md"].join("\n"));
...
test("evaluateVisualAssetsContract catches missing package framework/scripts", () => {
  const root = fixtureRepo();
  try {
