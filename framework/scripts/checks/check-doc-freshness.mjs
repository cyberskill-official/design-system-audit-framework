 ...
 *
 * Output: outputs/_audit/doc-freshness.json + a markdown report stub.
 *
 * Zero-dep Node ESM.
 */
...
const RFCS = resolve(ROOT, 'internal/rfcs');
const OUT_JSON = resolve(ROOT, 'outputs/_audit/doc-freshness.json');
const OUT_MD = resolve(ROOT, 'outputs/_audit/doc-freshness.md');
...
const partFiles = [
  ...readdirSync(FRAMEWORK_DIR).map(f => ({ name: f, dir: FRAMEWORK_DIR })),
  ...readdirSync(GUIDELINES_DIR).map(f => ({ name: f, dir: GUIDELINES_DIR }))
]
  .filter(({ name }) => (/^\d\d-/.test(name) || name.startsWith('part-')) && name.endsWith('.md'))
  .sort((a, b) => a.name.localeCompare(b.name));

const partNames = partFiles.map(p => p.name);
...
function countXrefs(content, currentDir) {
  const linkRe = /\]\(([^)]+\.md)(?:#[^)]*)?\)/g;
  let total = 0;
  let broken = 0;
  let m;
  while ((m = linkRe.exec(content)) !== null) {
    ...
    if (targetBase.endsWith('.md') && !partNames.includes(targetBase)) {
      // Check if resolved path exists
      const targetAbs = resolve(currentDir, m[1]);
      if (!existsSync(targetAbs)) broken++;
    }
  }
  return { total, broken };
}
...
for (const f of partFiles) {
  ...
  for (const [rfcFile, rfcSrc] of Object.entries(rfcContents)) {
    if (rfcSrc.includes(f.name) || rfcSrc.includes(f.name.replace('.md', ''))) {
