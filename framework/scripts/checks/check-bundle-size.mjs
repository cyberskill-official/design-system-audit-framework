 ...
 *
 * Output: outputs/_audit/bundle-size.json (trended over time)
 *
 * Zero-dep Node ESM.
 */
...
const PKGS = resolve(ROOT, 'framework/packages');
const OUT = resolve(ROOT, 'outputs/_audit/bundle-size.json');
...
if (!existsSync(PKGS)) {
  ...
  const summary = {
    ...
    reason: 'No framework/packages/ directory in this repository; run this check inside a packaged design-system repo.',
    total: {},
  ...
  if (opts.json) process.stdout.write(JSON.stringify(summary, null, 2) + '\n');
    ...
    console.log('[check-bundle-size] No framework/packages/ directory; wrote not-applicable result.');
    console.log('Output: outputs/_audit/bundle-size.json');
  }
  process.exit(0);
}
