...
function fixtureRepo() {
  ...
  mkdirSync(join(root, "outputs/_audit"), { recursive: true });
  ...
  mkdirSync(join(root, "internal/landing/benchmark"), { recursive: true });
  mkdirSync(join(root, "internal/landing/benchmark/privacy"), { recursive: true });

  writeFileSync(join(root, "internal/branding/brand-decoupling-domain-decision.md"), [
  ...
  writeFileSync(join(root, "internal/ADR-FR-BRAND-004.md"), "FR-BRAND-004\n\n**Status:** accepted\n\n```mermaid\nflowchart TD\nA-->B\n```\n");
  writeFileSync(join(root, ".github/CODEOWNERS"), payload.required_codeowners.join("\n"));
  writeFileSync(join(root, "internal/landing/vercel.json"), JSON.stringify({ headers: [{ headers: [{ key: "Strict-Transport-Security" }, { key: "Content-Security-Policy" }] }] }));
  writeFileSync(join(root, "README.md"), "https://audit.cyberskill.world\ninternal/SERVICES.md\nmethodology surface stays neutral\n");
  writeFileSync(join(root, "internal/landing/index.html"), "<link rel=\"canonical\" href=\"https://audit.cyberskill.world/\"><meta property=\"og:url\" content=\"https://audit.cyberskill.world/\">DSAF — Design System Audit Framework");
  writeFileSync(join(root, "internal/landing/card/index.html"), "<link rel=\"canonical\" href=\"https://audit.cyberskill.world/card\">DSAF-25 Core card");
  writeFileSync(join(root, "internal/landing/benchmark/index.html"), "<link rel=\"canonical\" href=\"https://audit.cyberskill.world/benchmark/\">Benchmark");
  writeFileSync(join(root, "internal/landing/benchmark/results.html"), "<link rel=\"canonical\" href=\"https://audit.cyberskill.world/benchmark/results.html\">Results");
  writeFileSync(join(root, "internal/landing/benchmark/privacy/index.html"), "<link rel=\"canonical\" href=\"https://audit.cyberskill.world/benchmark/privacy/\">info@cyberskill.world");
  writeFileSync(join(root, "internal/landing/README.md"), "`https://audit.cyberskill.world`\nNo paid-service CTA, pricing, or sales form lives here.\nNo redirect rules ship from here\n");
...
test("walkActiveSurfaces includes active internal/landing files", () => {
  ...
  try {
    mkdirSync(join(root, "internal/landing", "dist"), { recursive: true });
    mkdirSync(join(root, "internal/landing", ".cache"), { recursive: true });
    writeFileSync(join(root, "internal/landing", "dist", "ignored.html"), "https://dsaf.dev");
    writeFileSync(join(root, "internal/landing", ".cache", "ignored.html"), "https://dsaf.dev");
    writeFileSync(join(root, "internal/landing", "ignored.json"), "{\"url\":\"https://dsaf.dev\"}");
    const files = walkActiveSurfaces(root, payload).map((file) => file.replace(root + "/", ""));
    assert.ok(files.includes("README.md"));
    assert.ok(files.includes("internal/landing/index.html"));
    assert.ok(!files.includes("internal/landing/dist/ignored.html"));
    assert.ok(!files.includes("internal/landing/.cache/ignored.html"));
    assert.ok(!files.includes("internal/landing/ignored.json"));
  }
  finally {
...
test("evaluateDecouplingContract catches stale neutral-domain links and sales copy", () => {
  const root = fixtureRepo();
  try {
    writeFileSync(join(root, "internal/landing/benchmark/results.html"), "See https://dsaf.dev and book a paid audit.");
...
test("evaluateDecouplingContract catches redirect config and malformed inventory", () => {
  const root = fixtureRepo();
  try {
    writeFileSync(join(root, "internal/landing/vercel.json"), JSON.stringify({ redirects: [{ source: "/x", destination: "/y" }], headers: [] }));
    writeFileSync(join(root, "internal/branding/brand-decoupling-domain-decision.md"), "no rows");
    const failures = evaluateDecouplingContract(payload, root).results.filter((item) => item.status === "fail");
...
test("evaluateDecouplingContract catches missing governance and package framework/scripts", () => {
  const root = fixtureRepo();
  try {
