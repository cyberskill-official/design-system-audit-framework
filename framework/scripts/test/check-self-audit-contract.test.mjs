...
} from "../lib/self-audit-contract-lib.mjs";
...
function fixtureRepo() {
  ...
  const files = [
    ...
    "guidelines/prompts",
    "framework/templates",
    "framework/examples/cyberskill-design-system",
  ];
  ...
  writeFileSync(join(root, "internal/SERVICES.md"), "CyberSkill cites the worked example at L3.\n");
  writeFileSync(join(root, "internal/branding/self-audit-policy.md"), "No third-party verification -> L3. Combined percentages may appear inside audit data tables.\n");
  writeFileSync(join(root, "guidelines/01-introduction.md"), "## The self-audit publication cap\nThis caps at L3.\n");
  writeFileSync(join(root, "framework/07-maturity-tiers.md"), "## Self-audit cap rule\nL3 (Managed) maximum without third-party verification.\n");
  writeFileSync(join(root, "framework/examples/cyberskill-design-system/README.md"), "Public cited level: L3 Managed.\n");
  writeFileSync(join(root, "framework/examples/cyberskill-design-system/improvement-plan.md"), "Worked Example Audit (L3 self-audit, uncertified)\nCited public Level\nL3 (Managed)\n");
  writeFileSync(join(root, "framework/examples/cyberskill-design-system/_history.md"), "L3 capped\nself-audit cap\n");
  writeFileSync(join(root, "framework/examples/cyberskill-design-system/audit-report-2026-04-27.md"), "combined: 84.6\ntier: L5 Optimised\n| Post-audit combined score | **84.6%** (L5 Optimised) |\n");
  return root;
}
...
test("walkPublicFiles excludes the preserved interior audit", () => {
  ...
  try {
    ...
    assert.ok(!files.includes("framework/examples/cyberskill-design-system/audit-report-2026-04-27.md"));
  }
  finally {
...
test("evaluateSelfAuditContract catches public CyberSkill L5 claims", () => {
  const root = fixtureRepo();
  try {
    writeFileSync(join(root, "internal/landing/index.html"), "CyberSkill reached L5 Optimised.");
    const evaluation = evaluateSelfAuditContract(payload, root);
    assert.ok(evaluation.results.some((item) => item.status === "fail" && item.file === "internal/landing/index.html"));
  }
  finally {
    rmSync(root, { recursive: true, force: true });
  }
