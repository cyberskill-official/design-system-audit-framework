...
<picture>
  ...
  <img src="./framework/assets/dsaf-l0-l5-ladder.svg" alt="DSAF Levels ladder" width="100%">
</picture>

<picture>
  <source srcset="./framework/assets/dsaf-radar-dark.svg" media="(prefers-color-scheme: dark)">
  <img src="./framework/assets/dsaf-radar.svg" alt="DSAF radar chart" width="100%">
</picture>

**Read DSAF-25 Core first.** If you only have 5 minutes, [framework/dsaf-25.md](./framework/dsaf-25.md) is the 25-criterion subset that fits on one page. The full DSAF Criteria are in docs.
...
npm run verify
node framework/scripts/bin/audit-init.mjs /tmp/system
\`\`\`
...
| 2 | [framework/02-framework.md](./framework/02-framework.md) | Modes |
| 3 | [framework/dsaf-25.md](./framework/dsaf-25.md) | DSAF-25 Core |
| 4 | [guidelines/05-running-an-audit.md](./guidelines/05-running-an-audit.md) | Workflow |
| 5 | [framework/07-maturity-tiers.md](./framework/07-maturity-tiers.md) | DSAF Levels |
...
function fixtureRepo() {
  ...
  writeFileSync(join(root, "guidelines/01-introduction.md"), payload.reading_order.intro_required.join("\n"));
...
test("evaluateReadmeContract catches reading order and endorsement regressions", () => {
  ...
  try {
    ...
    writeFileSync(join(root, "guidelines/01-introduction.md"), "missing intro rows");
    const failures = evaluateReadmeContract(payload, root).results.filter((item) => item.status === "fail");
