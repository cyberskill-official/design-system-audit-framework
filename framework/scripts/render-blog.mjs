...
const blogDir = join(root, "internal/landing", "blog");
...
const deployAssetsDir = join(root, "internal/landing", "outputs/assets");
...
for (const asset of [
  ...
  copyFileSync(join(root, "outputs/assets", asset), join(deployAssetsDir, asset));
  console.log(`[render-blog] framework/assets/${asset} -> internal/landing/assets/${asset}`);
}
...
for (const asset of [
  ...
  const source = join(root, "outputs/assets", "og", asset);
  if (existsSync(source)) {
    copyFileSync(source, join(deployOgDir, asset));
    console.log(`[render-blog] framework/assets/og/${asset} -> internal/landing/assets/og/${asset}`);
  }
}
...
const deepDiveSourceDir = join(root, 'internal/content/deep-dives');
