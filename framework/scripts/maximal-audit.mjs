...
const CRITERIA_FILES = ["framework/03-criteria-part-a.md", "framework/04-criteria-part-b.md"];

const EVIDENCE_SOURCES = [
  ["DSAF-A", "DSAF Part A criteria", "local source", "framework/03-criteria-part-a.md", "2026-05-24", "A"],
  ["DSAF-B", "DSAF Part B criteria", "local source", "framework/04-criteria-part-b.md", "2026-05-24", "A"],
...
const ENTERPRISE_CATEGORIES = [
  ...
  ["Migration and change management", ["Migration plans define inventory, impact analysis, codemods, design-file updates, docs updates, training, support, and rollback.", "Breaking changes include rationale, alternatives, timeline, affected components, affected tokens, risk, and migration outputs/examples.", "Deprecation policy defines warning period, replacement, lint rule, migration script, support window, and removal version.", "Legacy compatibility mode identifies what is supported, what is frozen, what is unsafe, and what requires exception approval.", "Change adoption is measured through import changes, token usage, docs traffic, migration PRs, support tickets, and defects.", "High-risk migrations require pilot teams, staged rollout, feature flags, visual diff review, and executive risk acceptance.", "Design-file migrations are handled with the same seriousness as code migrations.", "Post-migration review verifies accessibility, localization, performance, brand, analytics, and product workflow outcomes."]],
  ["Documentation, education, and community", ["Documentation IA supports getting started, foundations, components, patterns, tokens, accessibility, content, governance, contributing, and changelog.", "Each component doc includes overview, anatomy, API, usage, states, accessibility, localization, tokens, examples, anti-patterns, and test guidance.", "Docs outputs/examples run against current framework/packages or are clearly marked illustrative until framework/packages exist.", "Search, navigation, related links, version switcher, status badges, and source links help readers find trustworthy guidance quickly.", "Contribution docs explain issue templates, proposal path, review criteria, acceptance gates, and code of conduct.", "Community and customer feedback is triaged into bugs, gaps, feature requests, research needs, legal questions, and roadmap candidates.", "Education material includes onboarding, workshops, migration guides, office hours, examples, and release briefings.", "Docs quality is measured through task success, time to first contribution, search success, support tickets, and reader feedback."]],
  ["Native, mobile, embedded, and multi-surface UX", ["Mobile guidance covers touch targets, safe areas, gestures, keyboard, orientation, offline, permissions, and platform conventions.", "Native guidance defines parity and divergence between web, iOS, Android, React Native, Flutter, and platform-specific controls.", "Embedded surfaces define iframe, app shell, host navigation, permissions, error states, theming, and degraded mode.", "Email, PDF, print, docs, admin, dashboard, and notification surfaces have explicit component and token compatibility rules.", "Responsive outputs/examples show dense enterprise use cases, not only marketing layouts.", "Device support matrix includes browser, OS, assistive tech, viewport, input modality, and performance constraints.", "Cross-surface analytics distinguish where a pattern succeeds or fails by device, locale, role, and workflow.", "Platform-specific visual effects such as glass, blur, haptics, and native motion have fallback and accessibility rules."]],
  ...
  ["Cybersecurity, risk, and critical operations UX", [
    "Cybersecurity threat level indicators and system status alerts use standardized severity colors, icons, and persistent visibility rules.",
    "Risk representation patterns distinguish assessed risk, residual risk, compliance status, and active vulnerabilities with confidence tiers.",
    "Security audit logs and telemetry visualizers support human-readable summaries, filtering, exporting, and actor verification.",
    "Phishing simulation and training components include clear disclosures, educational explanations, and safe reporting inputs.",
    "Multi-factor authentication (MFA) and step-up authentication flows define recovery code UI, hardware key support, and timeout boundaries.",
    "Threat modeling workspace components support data flow diagramming, trust boundary lines, and vulnerability card layouts.",
    "Access control and permission assignment interfaces present explicit scope inheritance, role boundaries, and blast-radius warnings.",
    "Security incident response templates define banner hierarchies, status-page widgets, and emergency action buttons."
  ]],
  ["Developer tools, CLI, and terminal UX", [
    "Monospace typography and terminal-like UI panels are reserved for code, inputs, data tables, and CLI output streams.",
    "Subtle background grain and noise textures are calibrated for contrast safety and performance across light and dark modes.",
    "Command execution feeds expose execution logs, progress indicators, abort triggers, and copyable script commands.",
    "Keyboard-only navigation supports quick palettes, command shortcuts, and focus indicators designed for power users.",
    "Telemetry opt-in/opt-out forms require explicit user choice, privacy disclosures, and data-sharing granularity.",
    "Monospace data tables support tabular numbers, custom sorting, dense grids, and scroll indicators on mobile viewports.",
    "CLI configuration and installer documentation provides copyable scripts, manual verify checks, and OS-specific tabs.",
    "AI command generation UI exposes prompt adjustments, preview diffs, risk warnings, and dry-run execution actions."
  ]],
  ["Editorial, high-variance, and creative UX design", [
    "Inline media typography punctuation embeds small, rounded images inside headlines to serve as visual rhythm markers.",
    "Asymmetric layout systems define left-aligned headlines, split screens, and unequal grid grids to avoid symmetric patterns.",
    "Variance dial settings control layouts, asymmetry offsets, and element spacing to scale design expression from Swiss to Artsy.",
    "Density and creativity controls dynamically adjust margin, padding, typography sizes, and color saturation boundaries.",
    "Motion intent settings regulate spring physics, mounting animations, cascade stagger delays, and low-power performance budgets.",
    "Modern serif web typography is limited to premium editorial headers and is banned from dashboard table views.",
    "Visual contrast standards for luxury brand surfaces require HSL-tailored grays and forbid oversaturated gradients.",
    "Anti-template guidelines prohibit generic filler text, scroll indicators, and common three-column feature cards."
  ]],
  ["Travel, hospitality, and map-centric UX", [
    "Interactive geographic maps support keyboard panning, screen-reader marker summaries, zoom limits, and high-contrast boundaries.",
    "Date-range pickers for reservations display localized calendars, holiday indicators, checkout policies, and clear duration math.",
    "Multi-currency and price display components support localized formatting, fee breakdowns, tax exclusions, and instant conversion.",
    "Map marker components use color-safe status indicators, screen-reader accessible badges, and readable text sizes.",
    "Travel guidelines and safety disclosures are placed contextually in booking flows with translation options.",
    "Review and feedback forms support star ratings with screen-reader labels, text descriptions, and photos uploads.",
    "Booking status timelines show checkout countdowns, check-in instructions, host contact triggers, and cancellation rules.",
    "Local address and name formats adjust dynamically to the user locale, postcode validation, and country-specific rules."
  ]],
  ["Hardware-integrated and platform-native UX", [
    "Native accessibility API mappings translate web ARIA roles to iOS accessibility elements and Android accessibility nodes.",
    "Haptic feedback guidelines specify spring-based tactile vibration patterns for buttons, alerts, and navigation gestures.",
    "Dynamic screen constraints (such as notches, bezels, and dynamic islands) have safe area paddings defined in tokens.",
    "Multi-device handoff state indicators show connection strength, active companion device, and synchronization progress.",
    "System-level dark mode sync matches operating system appearance preferences and handles forced brightness levels.",
    "Offline and low-power rendering modes disable expensive GPU effects, shadows, background blur, and animation loops.",
    "Platform-specific controls (like Apple Liquid Glass or Android Material ripple) have CSS web equivalents mapped in components.",
    "Print-friendly CSS and PDF export stylesheets render clean black-and-white layouts, hiding navigation menus and banners."
  ]],
  ["Figma libraries and handoff tools design", [
    "Figma plugin UI constraints limit iframe sizes, match the Host theme, and enforce single-page scroll rules.",
    "Figma variables structure aligns naming, modes, and density layers directly with repository token JSON files.",
    "Handoff annotations capture responsive layouts, content constraints, keyboard models, and localization margins.",
    "Token-sync plugin workflows provide change logs, diff previews, branch selection, and manual resolution rules.",
    "Parity check scripts verify that figma component visual properties match live React code component variables.",
    "Figma library updates publish change logs, breaking migrations, and deprecated components with version alignment.",
    "Design plugins have owner names, installation steps, limits, and validation reports posted in documentation.",
    "Designer workflow checklists require manual handoff walkthroughs before declaring components production-ready."
  ]],
  ["Agile productivity and issue-tracking UX", [
    "Keyboard command menu (command palette) supports fuzzy searching, shortcut hints, recent actions, and custom triggers.",
    "Issue lists support dense rows, status icon indicators, priority labels, assignee avatar tags, and multi-select actions.",
    "Drag-and-drop board columns support keyboard-only reordering, focus states, drag handle indicators, and scroll-to-drag.",
    "Keyboard shortcuts are documented in a contextual modal with custom remap options and master toggle.",
    "Sub-task and dependency lists support nesting levels, status inheritance, blocker warnings, and creation flows.",
    "Activity history timelines display creation event, status change logs, description diffs, and comments.",
    "Issue creation dialogs support markdown editor fields, drag-and-drop image uploads, and quick-add templates.",
    "Filter bar components support multi-value criteria, saved views, query sharing, and clear-all triggers."
  ]],
  ["Collaboration and concurrent editing UX", [
    "Real-time concurrent editor cursors display user names, colored tracks, focus elements, and hover details.",
    "User presence indicators show active status, last-seen timestamp, current workspace location, and avatar stack.",
    "Activity feed panels show recent modifications, comment mentions, unread badges, and quick-jump links.",
    "Notification preference grids allow per-channel tuning for emails, mobile pushes, desktop alerts, and weekly digests.",
    "Rich-text block editing layouts support drag handles, typeahead menus (/ commands), block type conversion, and inline styling.",
    "Workspace sharing and permissions dialogs list group access, guest invites, link sharing levels, and access expiry.",
    "Document version history displays timeline logs, author names, restore page triggers, and inline deletion diffs.",
    "Collaborative comment blocks support rich text, emoji reactions, resolution status, and email notification settings."
  ]],
  ["AI code editor and agentic interface UX", [
    "AI chat sidebars display agent status, model info, token cost warnings, and citation link summaries.",
    "Inline AI code generation supports split-view diffs, reject actions, apply triggers, and line-by-line comments.",
    "AI context management lets users include file references, folder paths, workspace docs, and URL links.",
    "Agent execution dashboards show active tool call steps, file access logs, terminal run outputs, and pause buttons.",
    "AI response feedback controls include upvote/downvote buttons, copy options, and rate limit indicators.",
    "Prompt template managers support customized prompts, system context overrides, and variable insertions.",
    "AI-generated code warnings disclose non-production quality, license compliance, and safety review triggers.",
    "Agent permission policies control terminal command execution, file writing, web requests, and code generation limits."
  ]],
  ["Mainframe, cloud, and hybrid enterprise UX", [
    "High-density mainframe data tables support virtualization, server sorting, column locking, resize, and custom exports.",
    "Data lineage flow diagrams visualize data sources, ETL transformation nodes, system endpoints, and owner details.",
    "Export audit logs record file format, row counts, user identity, data classification, and retrieval reason.",
    "Carbon-neutral computing dashboards alert users to data-center energy efficiency, green hours, and server usage.",
    "Multi-system integration adapters display connection health, synchronization logs, API endpoints, and fallback plans.",
    "Legacy application migration panels overlay older screens to guide users through modernized design equivalents.",
    "Enterprise admin consoles manage tenant configuration, billing tiers, SCIM sync settings, and security policy rules.",
    "Cloud instance provisioning flows display cost estimation, resource capacity logs, region selection, and deployment progress."
  ]]
];

function usage() {
  return [
    "Usage: node framework/scripts/maximal-audit.mjs --input <DESIGN.md|url> --out <dir> [--mode analyze|improve|both] [--model <id>] [--max-pages 8]",
    "",
    "Outputs:",
